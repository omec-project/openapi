...
func (dst *LinksValueSchema) UnmarshalJSON(data []byte) error {
	...
	err = openapi.NewStrictDecoder(data).Decode(&dst.Link)
	if err == nil {
	...
	err = openapi.NewStrictDecoder(data).Decode(&dst.ArrayOfLink)
	if err == nil {
	...
	if match > 1 { // more than 1 match
		...
		return openapi.ReportError("data matches more than one schema in oneOf(LinksValueSchema)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return openapi.ReportError("data failed to match schemas in oneOf(LinksValueSchema)")
	}
}
...
func (obj *LinksValueSchema) GetActualInstance() any {
	if obj == nil {
		return nil
	}
	if obj.Link != nil {
...
func (obj LinksValueSchema) GetActualInstanceValue() any {
	if obj.Link != nil {
		return *obj.Link
	}
