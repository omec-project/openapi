...
func (dst *SteeringContainer) UnmarshalJSON(data []byte) error {
	...
	err = openapi.NewStrictDecoder(data).Decode(&dst.ArrayOfSteeringInfo)
	if err == nil {
	...
	if match > 1 { // more than 1 match
		...
		return openapi.ReportError("data matches more than one schema in oneOf(SteeringContainer)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return openapi.ReportError("data failed to match schemas in oneOf(SteeringContainer)")
	}
}
...
func (obj *SteeringContainer) GetActualInstance() any {
	if obj == nil {
		return nil
	}
...
func (obj SteeringContainer) GetActualInstanceValue() any {
	if obj.ArrayOfSteeringInfo != nil {
		return *obj.ArrayOfSteeringInfo
	}
