...
// checks if the GeographicArea type satisfies the MappedNullable interface at compile time
var _ openapi.MappedNullable = &GeographicArea{}
...
func (dst *GeographicArea) UnmarshalJSON(data []byte) error {
	...
	return openapi.ReportError("data failed to match schemas in anyOf(GeographicArea)")
}
...
func (src GeographicArea) ToMap() (map[string]any, error) {
	if src.EllipsoidArc != nil {
		return src.EllipsoidArc.ToMap()
	}

	if src.Local2dPointUncertaintyEllipse != nil {
		return src.Local2dPointUncertaintyEllipse.ToMap()
	}

	if src.Local3dPointUncertaintyEllipsoid != nil {
		return src.Local3dPointUncertaintyEllipsoid.ToMap()
	}

	if src.Point != nil {
		return src.Point.ToMap()
	}

	if src.PointAltitude != nil {
		return src.PointAltitude.ToMap()
	}

	if src.PointAltitudeUncertainty != nil {
		return src.PointAltitudeUncertainty.ToMap()
	}

	if src.PointUncertaintyCircle != nil {
		return src.PointUncertaintyCircle.ToMap()
	}

	if src.PointUncertaintyEllipse != nil {
		return src.PointUncertaintyEllipse.ToMap()
	}

	if src.Polygon != nil {
		return src.Polygon.ToMap()
	}
