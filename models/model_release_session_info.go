// Copyright (C) 2026 Intel Corporation
// SPDX-License-Identifier: Apache-2.0

/*
Namf_Communication

AMF Communication Service.
© 2024, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).
All rights reserved.

API version: 1.3.0-alpha.6

Authors: Aether SD-Core team
Contact: dev@lists.aetherproject.org

Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.
*/

package models

import (
	"encoding/json"

	"github.com/omec-project/openapi/v2"
)

// checks if the ReleaseSessionInfo type satisfies the MappedNullable interface at compile time
var _ openapi.MappedNullable = &ReleaseSessionInfo{}

// ReleaseSessionInfo PDU session Id(s) and the cause for triggering the release.
type ReleaseSessionInfo struct {
	ReleaseSessionList []int32      `json:"releaseSessionList" yaml:"releaseSessionList"`
	ReleaseCause       ReleaseCause `json:"releaseCause" yaml:"releaseCause"`
}

// NewReleaseSessionInfo instantiates a new ReleaseSessionInfo object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewReleaseSessionInfo(releaseSessionList []int32, releaseCause ReleaseCause) *ReleaseSessionInfo {
	this := ReleaseSessionInfo{}
	this.ReleaseSessionList = releaseSessionList
	this.ReleaseCause = releaseCause
	return &this
}

// NewReleaseSessionInfoWithDefaults instantiates a new ReleaseSessionInfo object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewReleaseSessionInfoWithDefaults() *ReleaseSessionInfo {
	this := ReleaseSessionInfo{}
	return &this
}

// GetReleaseSessionList returns the ReleaseSessionList field value
func (o *ReleaseSessionInfo) GetReleaseSessionList() []int32 {
	if o == nil {
		var ret []int32
		return ret
	}

	return o.ReleaseSessionList
}

// GetReleaseSessionListOk returns a tuple with the ReleaseSessionList field value
// and a boolean to check if the value has been set.
func (o *ReleaseSessionInfo) GetReleaseSessionListOk() ([]int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.ReleaseSessionList, true
}

// SetReleaseSessionList sets field value
func (o *ReleaseSessionInfo) SetReleaseSessionList(v []int32) {
	o.ReleaseSessionList = v
}

// GetReleaseCause returns the ReleaseCause field value
func (o *ReleaseSessionInfo) GetReleaseCause() ReleaseCause {
	if o == nil {
		var ret ReleaseCause
		return ret
	}

	return o.ReleaseCause
}

// GetReleaseCauseOk returns a tuple with the ReleaseCause field value
// and a boolean to check if the value has been set.
func (o *ReleaseSessionInfo) GetReleaseCauseOk() (*ReleaseCause, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ReleaseCause, true
}

// SetReleaseCause sets field value
func (o *ReleaseSessionInfo) SetReleaseCause(v ReleaseCause) {
	o.ReleaseCause = v
}

func (o ReleaseSessionInfo) ToMap() (map[string]any, error) {
	toSerialize := map[string]any{}
	toSerialize["releaseSessionList"] = o.ReleaseSessionList
	toSerialize["releaseCause"] = o.ReleaseCause
	return toSerialize, nil
}

type NullableReleaseSessionInfo struct {
	value *ReleaseSessionInfo
	isSet bool
}

func (v NullableReleaseSessionInfo) Get() *ReleaseSessionInfo {
	return v.value
}

func (v *NullableReleaseSessionInfo) Set(val *ReleaseSessionInfo) {
	v.value = val
	v.isSet = true
}

func (v NullableReleaseSessionInfo) IsSet() bool {
	return v.isSet
}

func (v *NullableReleaseSessionInfo) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableReleaseSessionInfo(val *ReleaseSessionInfo) *NullableReleaseSessionInfo {
	return &NullableReleaseSessionInfo{value: val, isSet: true}
}

func (v NullableReleaseSessionInfo) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableReleaseSessionInfo) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
