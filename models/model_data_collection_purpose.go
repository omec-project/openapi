// Copyright (C) 2026 Intel Corporation
// SPDX-License-Identifier: Apache-2.0

/*
Nnwdaf_DataManagement

Nnwdaf_DataManagement API Service.
© 2023, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).
All rights reserved.

API version: 1.1.0-alpha.4

Authors: Aether SD-Core team
Contact: dev@lists.aetherproject.org

Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.
*/

package models

import (
	"encoding/json"

	"github.com/omec-project/openapi/v2"
)

// DataCollectionPurpose Represents the purpose for data collection. Possible values are: - ANALYTICS_GENERATION: The data is collected for generating the analytics. - MODEL_TRAINING: The data is collected for ML model training.
type DataCollectionPurpose struct {
	String *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *DataCollectionPurpose) UnmarshalJSON(data []byte) error {
	// try to unmarshal JSON data into String
	err := json.Unmarshal(data, &dst.String)
	if err == nil {
		jsonString, _ := json.Marshal(dst.String)
		if string(jsonString) == "{}" { // empty struct
			dst.String = nil
		} else {
			return nil // data stored in dst.String, return on the first match
		}
	} else {
		dst.String = nil
	}

	return openapi.ReportError("data failed to match schemas in anyOf(DataCollectionPurpose)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src DataCollectionPurpose) MarshalJSON() ([]byte, error) {
	if src.String != nil {
		return json.Marshal(&src.String)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableDataCollectionPurpose struct {
	value *DataCollectionPurpose
	isSet bool
}

func (v NullableDataCollectionPurpose) Get() *DataCollectionPurpose {
	return v.value
}

func (v *NullableDataCollectionPurpose) Set(val *DataCollectionPurpose) {
	v.value = val
	v.isSet = true
}

func (v NullableDataCollectionPurpose) IsSet() bool {
	return v.isSet
}

func (v *NullableDataCollectionPurpose) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDataCollectionPurpose(val *DataCollectionPurpose) *NullableDataCollectionPurpose {
	return &NullableDataCollectionPurpose{value: val, isSet: true}
}

func (v NullableDataCollectionPurpose) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDataCollectionPurpose) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
