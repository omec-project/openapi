// Copyright (C) 2026 Intel Corporation
// SPDX-License-Identifier: Apache-2.0

/*
Npcf_SMPolicyControl API

Session Management Policy Control Service
© 2024, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).
All rights reserved.

API version: 1.3.0-alpha.6

Authors: Aether SD-Core team
Contact: dev@lists.aetherproject.org

Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.
*/

package models

import (
	"encoding/json"

	"github.com/omec-project/openapi/v2"
)

// checks if the UeInitiatedResourceRequest type satisfies the MappedNullable interface at compile time
var _ openapi.MappedNullable = &UeInitiatedResourceRequest{}

// UeInitiatedResourceRequest Indicates that a UE requests specific QoS handling for the selected SDF.
type UeInitiatedResourceRequest struct {
	PccRuleId    *string            `json:"pccRuleId,omitempty" yaml:"pccRuleId,omitempty"`
	RuleOp       RuleOperation      `json:"ruleOp" yaml:"ruleOp"`
	Precedence   *int32             `json:"precedence,omitempty" yaml:"precedence,omitempty"`
	PackFiltInfo []PacketFilterInfo `json:"packFiltInfo" yaml:"packFiltInfo"`
	ReqQos       *RequestedQos      `json:"reqQos,omitempty" yaml:"reqQos,omitempty"`
}

// NewUeInitiatedResourceRequest instantiates a new UeInitiatedResourceRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUeInitiatedResourceRequest(ruleOp RuleOperation, packFiltInfo []PacketFilterInfo) *UeInitiatedResourceRequest {
	this := UeInitiatedResourceRequest{}
	this.RuleOp = ruleOp
	this.PackFiltInfo = packFiltInfo
	return &this
}

// NewUeInitiatedResourceRequestWithDefaults instantiates a new UeInitiatedResourceRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUeInitiatedResourceRequestWithDefaults() *UeInitiatedResourceRequest {
	this := UeInitiatedResourceRequest{}
	return &this
}

// GetPccRuleId returns the PccRuleId field value if set, zero value otherwise.
func (o *UeInitiatedResourceRequest) GetPccRuleId() string {
	if o == nil || openapi.IsNil(o.PccRuleId) {
		var ret string
		return ret
	}
	return *o.PccRuleId
}

// GetPccRuleIdOk returns a tuple with the PccRuleId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UeInitiatedResourceRequest) GetPccRuleIdOk() (*string, bool) {
	if o == nil || openapi.IsNil(o.PccRuleId) {
		return nil, false
	}
	return o.PccRuleId, true
}

// HasPccRuleId returns a boolean if a field has been set.
func (o *UeInitiatedResourceRequest) HasPccRuleId() bool {
	if o != nil && !openapi.IsNil(o.PccRuleId) {
		return true
	}

	return false
}

// SetPccRuleId gets a reference to the given string and assigns it to the PccRuleId field.
func (o *UeInitiatedResourceRequest) SetPccRuleId(v string) {
	o.PccRuleId = &v
}

// GetRuleOp returns the RuleOp field value
func (o *UeInitiatedResourceRequest) GetRuleOp() RuleOperation {
	if o == nil {
		var ret RuleOperation
		return ret
	}

	return o.RuleOp
}

// GetRuleOpOk returns a tuple with the RuleOp field value
// and a boolean to check if the value has been set.
func (o *UeInitiatedResourceRequest) GetRuleOpOk() (*RuleOperation, bool) {
	if o == nil {
		return nil, false
	}
	return &o.RuleOp, true
}

// SetRuleOp sets field value
func (o *UeInitiatedResourceRequest) SetRuleOp(v RuleOperation) {
	o.RuleOp = v
}

// GetPrecedence returns the Precedence field value if set, zero value otherwise.
func (o *UeInitiatedResourceRequest) GetPrecedence() int32 {
	if o == nil || openapi.IsNil(o.Precedence) {
		var ret int32
		return ret
	}
	return *o.Precedence
}

// GetPrecedenceOk returns a tuple with the Precedence field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UeInitiatedResourceRequest) GetPrecedenceOk() (*int32, bool) {
	if o == nil || openapi.IsNil(o.Precedence) {
		return nil, false
	}
	return o.Precedence, true
}

// HasPrecedence returns a boolean if a field has been set.
func (o *UeInitiatedResourceRequest) HasPrecedence() bool {
	if o != nil && !openapi.IsNil(o.Precedence) {
		return true
	}

	return false
}

// SetPrecedence gets a reference to the given int32 and assigns it to the Precedence field.
func (o *UeInitiatedResourceRequest) SetPrecedence(v int32) {
	o.Precedence = &v
}

// GetPackFiltInfo returns the PackFiltInfo field value
func (o *UeInitiatedResourceRequest) GetPackFiltInfo() []PacketFilterInfo {
	if o == nil {
		var ret []PacketFilterInfo
		return ret
	}

	return o.PackFiltInfo
}

// GetPackFiltInfoOk returns a tuple with the PackFiltInfo field value
// and a boolean to check if the value has been set.
func (o *UeInitiatedResourceRequest) GetPackFiltInfoOk() ([]PacketFilterInfo, bool) {
	if o == nil {
		return nil, false
	}
	return o.PackFiltInfo, true
}

// SetPackFiltInfo sets field value
func (o *UeInitiatedResourceRequest) SetPackFiltInfo(v []PacketFilterInfo) {
	o.PackFiltInfo = v
}

// GetReqQos returns the ReqQos field value if set, zero value otherwise.
func (o *UeInitiatedResourceRequest) GetReqQos() RequestedQos {
	if o == nil || openapi.IsNil(o.ReqQos) {
		var ret RequestedQos
		return ret
	}
	return *o.ReqQos
}

// GetReqQosOk returns a tuple with the ReqQos field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *UeInitiatedResourceRequest) GetReqQosOk() (*RequestedQos, bool) {
	if o == nil || openapi.IsNil(o.ReqQos) {
		return nil, false
	}
	return o.ReqQos, true
}

// HasReqQos returns a boolean if a field has been set.
func (o *UeInitiatedResourceRequest) HasReqQos() bool {
	if o != nil && !openapi.IsNil(o.ReqQos) {
		return true
	}

	return false
}

// SetReqQos gets a reference to the given RequestedQos and assigns it to the ReqQos field.
func (o *UeInitiatedResourceRequest) SetReqQos(v RequestedQos) {
	o.ReqQos = &v
}

func (o UeInitiatedResourceRequest) ToMap() (map[string]any, error) {
	toSerialize := map[string]any{}
	if !openapi.IsNil(o.PccRuleId) {
		toSerialize["pccRuleId"] = o.PccRuleId
	}
	toSerialize["ruleOp"] = o.RuleOp
	if !openapi.IsNil(o.Precedence) {
		toSerialize["precedence"] = o.Precedence
	}
	toSerialize["packFiltInfo"] = o.PackFiltInfo
	if !openapi.IsNil(o.ReqQos) {
		toSerialize["reqQos"] = o.ReqQos
	}
	return toSerialize, nil
}

type NullableUeInitiatedResourceRequest struct {
	value *UeInitiatedResourceRequest
	isSet bool
}

func (v NullableUeInitiatedResourceRequest) Get() *UeInitiatedResourceRequest {
	return v.value
}

func (v *NullableUeInitiatedResourceRequest) Set(val *UeInitiatedResourceRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableUeInitiatedResourceRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableUeInitiatedResourceRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUeInitiatedResourceRequest(val *UeInitiatedResourceRequest) *NullableUeInitiatedResourceRequest {
	return &NullableUeInitiatedResourceRequest{value: val, isSet: true}
}

func (v NullableUeInitiatedResourceRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUeInitiatedResourceRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
