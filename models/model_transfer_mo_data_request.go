// Copyright (C) 2026 Intel Corporation
// SPDX-License-Identifier: Apache-2.0

/*
Nsmf_PDUSession

SMF PDU Session Service.
© 2024, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).
All rights reserved.

API version: 1.3.0-alpha.7

Authors: Aether SD-Core team
Contact: dev@lists.aetherproject.org

Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.
*/

package models

import (
	"encoding/json"
	"os"

	"github.com/omec-project/openapi/v2"
)

// checks if the TransferMoDataRequest type satisfies the MappedNullable interface at compile time
var _ openapi.MappedNullable = &TransferMoDataRequest{}

// TransferMoDataRequest struct for TransferMoDataRequest
type TransferMoDataRequest struct {
	JsonData     *TransferMoDataReqData `json:"jsonData,omitempty" yaml:"jsonData,omitempty" multipart:"contentType:application/json,ref:{jsonData}"`
	BinaryMoData **os.File              `json:"binaryMoData,omitempty" yaml:"binaryMoData,omitempty" multipart:"contentType:application/vnd.3gpp.5gnas,ref:{binaryMoData}"`
}

// NewTransferMoDataRequest instantiates a new TransferMoDataRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTransferMoDataRequest() *TransferMoDataRequest {
	this := TransferMoDataRequest{}
	return &this
}

// NewTransferMoDataRequestWithDefaults instantiates a new TransferMoDataRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTransferMoDataRequestWithDefaults() *TransferMoDataRequest {
	this := TransferMoDataRequest{}
	return &this
}

// GetJsonData returns the JsonData field value if set, zero value otherwise.
func (o *TransferMoDataRequest) GetJsonData() TransferMoDataReqData {
	if o == nil || openapi.IsNil(o.JsonData) {
		var ret TransferMoDataReqData
		return ret
	}
	return *o.JsonData
}

// GetJsonDataOk returns a tuple with the JsonData field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TransferMoDataRequest) GetJsonDataOk() (*TransferMoDataReqData, bool) {
	if o == nil || openapi.IsNil(o.JsonData) {
		return nil, false
	}
	return o.JsonData, true
}

// HasJsonData returns a boolean if a field has been set.
func (o *TransferMoDataRequest) HasJsonData() bool {
	if o != nil && !openapi.IsNil(o.JsonData) {
		return true
	}

	return false
}

// SetJsonData gets a reference to the given TransferMoDataReqData and assigns it to the JsonData field.
func (o *TransferMoDataRequest) SetJsonData(v TransferMoDataReqData) {
	o.JsonData = &v
}

// GetBinaryMoData returns the BinaryMoData field value if set, zero value otherwise.
func (o *TransferMoDataRequest) GetBinaryMoData() *os.File {
	if o == nil || openapi.IsNil(o.BinaryMoData) {
		var ret *os.File
		return ret
	}
	return *o.BinaryMoData
}

// GetBinaryMoDataOk returns a tuple with the BinaryMoData field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TransferMoDataRequest) GetBinaryMoDataOk() (**os.File, bool) {
	if o == nil || openapi.IsNil(o.BinaryMoData) {
		return nil, false
	}
	return o.BinaryMoData, true
}

// HasBinaryMoData returns a boolean if a field has been set.
func (o *TransferMoDataRequest) HasBinaryMoData() bool {
	if o != nil && !openapi.IsNil(o.BinaryMoData) {
		return true
	}

	return false
}

// SetBinaryMoData gets a reference to the given *os.File and assigns it to the BinaryMoData field.
func (o *TransferMoDataRequest) SetBinaryMoData(v *os.File) {
	o.BinaryMoData = &v
}

func (o TransferMoDataRequest) ToMap() (map[string]any, error) {
	toSerialize := map[string]any{}
	if !openapi.IsNil(o.JsonData) {
		toSerialize["jsonData"] = o.JsonData
	}
	if !openapi.IsNil(o.BinaryMoData) {
		toSerialize["binaryMoData"] = o.BinaryMoData
	}
	return toSerialize, nil
}

type NullableTransferMoDataRequest struct {
	value *TransferMoDataRequest
	isSet bool
}

func (v NullableTransferMoDataRequest) Get() *TransferMoDataRequest {
	return v.value
}

func (v *NullableTransferMoDataRequest) Set(val *TransferMoDataRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableTransferMoDataRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableTransferMoDataRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTransferMoDataRequest(val *TransferMoDataRequest) *NullableTransferMoDataRequest {
	return &NullableTransferMoDataRequest{value: val, isSet: true}
}

func (v NullableTransferMoDataRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTransferMoDataRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
