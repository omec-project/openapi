// Copyright (C) 2026 Intel Corporation
// SPDX-License-Identifier: Apache-2.0

/*
AUSF API

AUSF UE Authentication Service.
© 2023, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).
All rights reserved.

API version: 1.3.0-alpha.4

Authors: Aether SD-Core team
Contact: dev@lists.aetherproject.org

Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.
*/

package models

import (
	"encoding/json"

	"github.com/omec-project/openapi/v2"
)

// checks if the RgAuthenticationInfo type satisfies the MappedNullable interface at compile time
var _ openapi.MappedNullable = &RgAuthenticationInfo{}

// RgAuthenticationInfo Contains the UE id (i.e. SUCI) and the authenticated indication.
type RgAuthenticationInfo struct {
	// Contains the SUCI.
	Suci             string `json:"suci" yaml:"suci" validate:"regexp=^(suci-(0-[0-9]{3}-[0-9]{2,3}|[1-7]-.+)-[0-9]{1,4}-(0-0-.+|[a-fA-F1-9]-([1-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])-[a-fA-F0-9]+)|.+)$"`
	AuthenticatedInd bool   `json:"authenticatedInd" yaml:"authenticatedInd"`
	// A string used to indicate the features supported by an API that is used as defined in clause  6.6 in 3GPP TS 29.500. The string shall contain a bitmask indicating supported features in  hexadecimal representation Each character in the string shall take a value of \"0\" to \"9\",  \"a\" to \"f\" or \"A\" to \"F\" and shall represent the support of 4 features as described in  table 5.2.2-3. The most significant character representing the highest-numbered features shall  appear first in the string, and the character representing features 1 to 4 shall appear last  in the string. The list of features and their numbering (starting with 1) are defined  separately for each API. If the string contains a lower number of characters than there are  defined features for an API, all features that would be represented by characters that are not  present in the string are not supported.
	SupportedFeatures *string `json:"supportedFeatures,omitempty" yaml:"supportedFeatures,omitempty" validate:"regexp=^[A-Fa-f0-9]*$"`
}

// NewRgAuthenticationInfo instantiates a new RgAuthenticationInfo object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRgAuthenticationInfo(suci string, authenticatedInd bool) *RgAuthenticationInfo {
	this := RgAuthenticationInfo{}
	this.Suci = suci
	this.AuthenticatedInd = authenticatedInd
	return &this
}

// NewRgAuthenticationInfoWithDefaults instantiates a new RgAuthenticationInfo object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRgAuthenticationInfoWithDefaults() *RgAuthenticationInfo {
	this := RgAuthenticationInfo{}
	var authenticatedInd bool = false
	this.AuthenticatedInd = authenticatedInd
	return &this
}

// GetSuci returns the Suci field value
func (o *RgAuthenticationInfo) GetSuci() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Suci
}

// GetSuciOk returns a tuple with the Suci field value
// and a boolean to check if the value has been set.
func (o *RgAuthenticationInfo) GetSuciOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Suci, true
}

// SetSuci sets field value
func (o *RgAuthenticationInfo) SetSuci(v string) {
	o.Suci = v
}

// GetAuthenticatedInd returns the AuthenticatedInd field value
func (o *RgAuthenticationInfo) GetAuthenticatedInd() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.AuthenticatedInd
}

// GetAuthenticatedIndOk returns a tuple with the AuthenticatedInd field value
// and a boolean to check if the value has been set.
func (o *RgAuthenticationInfo) GetAuthenticatedIndOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.AuthenticatedInd, true
}

// SetAuthenticatedInd sets field value
func (o *RgAuthenticationInfo) SetAuthenticatedInd(v bool) {
	o.AuthenticatedInd = v
}

// GetSupportedFeatures returns the SupportedFeatures field value if set, zero value otherwise.
func (o *RgAuthenticationInfo) GetSupportedFeatures() string {
	if o == nil || openapi.IsNil(o.SupportedFeatures) {
		var ret string
		return ret
	}
	return *o.SupportedFeatures
}

// GetSupportedFeaturesOk returns a tuple with the SupportedFeatures field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RgAuthenticationInfo) GetSupportedFeaturesOk() (*string, bool) {
	if o == nil || openapi.IsNil(o.SupportedFeatures) {
		return nil, false
	}
	return o.SupportedFeatures, true
}

// HasSupportedFeatures returns a boolean if a field has been set.
func (o *RgAuthenticationInfo) HasSupportedFeatures() bool {
	if o != nil && !openapi.IsNil(o.SupportedFeatures) {
		return true
	}

	return false
}

// SetSupportedFeatures gets a reference to the given string and assigns it to the SupportedFeatures field.
func (o *RgAuthenticationInfo) SetSupportedFeatures(v string) {
	o.SupportedFeatures = &v
}

func (o RgAuthenticationInfo) ToMap() (map[string]any, error) {
	toSerialize := map[string]any{}
	toSerialize["suci"] = o.Suci
	toSerialize["authenticatedInd"] = o.AuthenticatedInd
	if !openapi.IsNil(o.SupportedFeatures) {
		toSerialize["supportedFeatures"] = o.SupportedFeatures
	}
	return toSerialize, nil
}

type NullableRgAuthenticationInfo struct {
	value *RgAuthenticationInfo
	isSet bool
}

func (v NullableRgAuthenticationInfo) Get() *RgAuthenticationInfo {
	return v.value
}

func (v *NullableRgAuthenticationInfo) Set(val *RgAuthenticationInfo) {
	v.value = val
	v.isSet = true
}

func (v NullableRgAuthenticationInfo) IsSet() bool {
	return v.isSet
}

func (v *NullableRgAuthenticationInfo) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRgAuthenticationInfo(val *RgAuthenticationInfo) *NullableRgAuthenticationInfo {
	return &NullableRgAuthenticationInfo{value: val, isSet: true}
}

func (v NullableRgAuthenticationInfo) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRgAuthenticationInfo) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
