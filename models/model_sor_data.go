// Copyright (C) 2026 Intel Corporation
// SPDX-License-Identifier: Apache-2.0

/*
Nudr_DataRepository API OpenAPI file

Unified Data Repository Service.
© 2024, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).
All rights reserved.

API version: 2.3.0-alpha.6

Authors: Aether SD-Core team
Contact: dev@lists.aetherproject.org

Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.
*/

package models

import (
	"encoding/json"
	"time"

	"github.com/omec-project/openapi/v2"
)

// checks if the SorData type satisfies the MappedNullable interface at compile time
var _ openapi.MappedNullable = &SorData{}

// SorData Used to store the status of the latest SOR data update.
type SorData struct {
	// string with format 'date-time' as defined in OpenAPI.
	ProvisioningTime time.Time      `json:"provisioningTime" yaml:"provisioningTime"`
	UeUpdateStatus   UeUpdateStatus `json:"ueUpdateStatus" yaml:"ueUpdateStatus"`
	// MAC value for protecting SOR procedure (SoR-MAC-IAUSF and SoR-XMAC-IUE).
	SorXmacIue *string `json:"sorXmacIue,omitempty" yaml:"sorXmacIue,omitempty" validate:"regexp=^[A-Fa-f0-9]{32}$"`
	// MAC value for protecting SOR procedure (SoR-MAC-IAUSF and SoR-XMAC-IUE).
	SorMacIue              *string `json:"sorMacIue,omitempty" yaml:"sorMacIue,omitempty" validate:"regexp=^[A-Fa-f0-9]{32}$"`
	MeSupportOfSorCmci     *bool   `json:"meSupportOfSorCmci,omitempty" yaml:"meSupportOfSorCmci,omitempty"`
	MeSupportOfSorSnpnSi   *bool   `json:"meSupportOfSorSnpnSi,omitempty" yaml:"meSupportOfSorSnpnSi,omitempty"`
	MeSupportOfSorSnpnSiLs *bool   `json:"meSupportOfSorSnpnSiLs,omitempty" yaml:"meSupportOfSorSnpnSiLs,omitempty"`
}

// NewSorData instantiates a new SorData object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSorData(provisioningTime time.Time, ueUpdateStatus UeUpdateStatus) *SorData {
	this := SorData{}
	this.ProvisioningTime = provisioningTime
	this.UeUpdateStatus = ueUpdateStatus
	var meSupportOfSorSnpnSi bool = false
	this.MeSupportOfSorSnpnSi = &meSupportOfSorSnpnSi
	var meSupportOfSorSnpnSiLs bool = false
	this.MeSupportOfSorSnpnSiLs = &meSupportOfSorSnpnSiLs
	return &this
}

// NewSorDataWithDefaults instantiates a new SorData object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSorDataWithDefaults() *SorData {
	this := SorData{}
	var meSupportOfSorSnpnSi bool = false
	this.MeSupportOfSorSnpnSi = &meSupportOfSorSnpnSi
	var meSupportOfSorSnpnSiLs bool = false
	this.MeSupportOfSorSnpnSiLs = &meSupportOfSorSnpnSiLs
	return &this
}

// GetProvisioningTime returns the ProvisioningTime field value
func (o *SorData) GetProvisioningTime() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.ProvisioningTime
}

// GetProvisioningTimeOk returns a tuple with the ProvisioningTime field value
// and a boolean to check if the value has been set.
func (o *SorData) GetProvisioningTimeOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ProvisioningTime, true
}

// SetProvisioningTime sets field value
func (o *SorData) SetProvisioningTime(v time.Time) {
	o.ProvisioningTime = v
}

// GetUeUpdateStatus returns the UeUpdateStatus field value
func (o *SorData) GetUeUpdateStatus() UeUpdateStatus {
	if o == nil {
		var ret UeUpdateStatus
		return ret
	}

	return o.UeUpdateStatus
}

// GetUeUpdateStatusOk returns a tuple with the UeUpdateStatus field value
// and a boolean to check if the value has been set.
func (o *SorData) GetUeUpdateStatusOk() (*UeUpdateStatus, bool) {
	if o == nil {
		return nil, false
	}
	return &o.UeUpdateStatus, true
}

// SetUeUpdateStatus sets field value
func (o *SorData) SetUeUpdateStatus(v UeUpdateStatus) {
	o.UeUpdateStatus = v
}

// GetSorXmacIue returns the SorXmacIue field value if set, zero value otherwise.
func (o *SorData) GetSorXmacIue() string {
	if o == nil || openapi.IsNil(o.SorXmacIue) {
		var ret string
		return ret
	}
	return *o.SorXmacIue
}

// GetSorXmacIueOk returns a tuple with the SorXmacIue field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SorData) GetSorXmacIueOk() (*string, bool) {
	if o == nil || openapi.IsNil(o.SorXmacIue) {
		return nil, false
	}
	return o.SorXmacIue, true
}

// HasSorXmacIue returns a boolean if a field has been set.
func (o *SorData) HasSorXmacIue() bool {
	if o != nil && !openapi.IsNil(o.SorXmacIue) {
		return true
	}

	return false
}

// SetSorXmacIue gets a reference to the given string and assigns it to the SorXmacIue field.
func (o *SorData) SetSorXmacIue(v string) {
	o.SorXmacIue = &v
}

// GetSorMacIue returns the SorMacIue field value if set, zero value otherwise.
func (o *SorData) GetSorMacIue() string {
	if o == nil || openapi.IsNil(o.SorMacIue) {
		var ret string
		return ret
	}
	return *o.SorMacIue
}

// GetSorMacIueOk returns a tuple with the SorMacIue field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SorData) GetSorMacIueOk() (*string, bool) {
	if o == nil || openapi.IsNil(o.SorMacIue) {
		return nil, false
	}
	return o.SorMacIue, true
}

// HasSorMacIue returns a boolean if a field has been set.
func (o *SorData) HasSorMacIue() bool {
	if o != nil && !openapi.IsNil(o.SorMacIue) {
		return true
	}

	return false
}

// SetSorMacIue gets a reference to the given string and assigns it to the SorMacIue field.
func (o *SorData) SetSorMacIue(v string) {
	o.SorMacIue = &v
}

// GetMeSupportOfSorCmci returns the MeSupportOfSorCmci field value if set, zero value otherwise.
func (o *SorData) GetMeSupportOfSorCmci() bool {
	if o == nil || openapi.IsNil(o.MeSupportOfSorCmci) {
		var ret bool
		return ret
	}
	return *o.MeSupportOfSorCmci
}

// GetMeSupportOfSorCmciOk returns a tuple with the MeSupportOfSorCmci field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SorData) GetMeSupportOfSorCmciOk() (*bool, bool) {
	if o == nil || openapi.IsNil(o.MeSupportOfSorCmci) {
		return nil, false
	}
	return o.MeSupportOfSorCmci, true
}

// HasMeSupportOfSorCmci returns a boolean if a field has been set.
func (o *SorData) HasMeSupportOfSorCmci() bool {
	if o != nil && !openapi.IsNil(o.MeSupportOfSorCmci) {
		return true
	}

	return false
}

// SetMeSupportOfSorCmci gets a reference to the given bool and assigns it to the MeSupportOfSorCmci field.
func (o *SorData) SetMeSupportOfSorCmci(v bool) {
	o.MeSupportOfSorCmci = &v
}

// GetMeSupportOfSorSnpnSi returns the MeSupportOfSorSnpnSi field value if set, zero value otherwise.
func (o *SorData) GetMeSupportOfSorSnpnSi() bool {
	if o == nil || openapi.IsNil(o.MeSupportOfSorSnpnSi) {
		var ret bool
		return ret
	}
	return *o.MeSupportOfSorSnpnSi
}

// GetMeSupportOfSorSnpnSiOk returns a tuple with the MeSupportOfSorSnpnSi field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SorData) GetMeSupportOfSorSnpnSiOk() (*bool, bool) {
	if o == nil || openapi.IsNil(o.MeSupportOfSorSnpnSi) {
		return nil, false
	}
	return o.MeSupportOfSorSnpnSi, true
}

// HasMeSupportOfSorSnpnSi returns a boolean if a field has been set.
func (o *SorData) HasMeSupportOfSorSnpnSi() bool {
	if o != nil && !openapi.IsNil(o.MeSupportOfSorSnpnSi) {
		return true
	}

	return false
}

// SetMeSupportOfSorSnpnSi gets a reference to the given bool and assigns it to the MeSupportOfSorSnpnSi field.
func (o *SorData) SetMeSupportOfSorSnpnSi(v bool) {
	o.MeSupportOfSorSnpnSi = &v
}

// GetMeSupportOfSorSnpnSiLs returns the MeSupportOfSorSnpnSiLs field value if set, zero value otherwise.
func (o *SorData) GetMeSupportOfSorSnpnSiLs() bool {
	if o == nil || openapi.IsNil(o.MeSupportOfSorSnpnSiLs) {
		var ret bool
		return ret
	}
	return *o.MeSupportOfSorSnpnSiLs
}

// GetMeSupportOfSorSnpnSiLsOk returns a tuple with the MeSupportOfSorSnpnSiLs field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SorData) GetMeSupportOfSorSnpnSiLsOk() (*bool, bool) {
	if o == nil || openapi.IsNil(o.MeSupportOfSorSnpnSiLs) {
		return nil, false
	}
	return o.MeSupportOfSorSnpnSiLs, true
}

// HasMeSupportOfSorSnpnSiLs returns a boolean if a field has been set.
func (o *SorData) HasMeSupportOfSorSnpnSiLs() bool {
	if o != nil && !openapi.IsNil(o.MeSupportOfSorSnpnSiLs) {
		return true
	}

	return false
}

// SetMeSupportOfSorSnpnSiLs gets a reference to the given bool and assigns it to the MeSupportOfSorSnpnSiLs field.
func (o *SorData) SetMeSupportOfSorSnpnSiLs(v bool) {
	o.MeSupportOfSorSnpnSiLs = &v
}

func (o SorData) ToMap() (map[string]any, error) {
	toSerialize := map[string]any{}
	toSerialize["provisioningTime"] = o.ProvisioningTime
	toSerialize["ueUpdateStatus"] = o.UeUpdateStatus
	if !openapi.IsNil(o.SorXmacIue) {
		toSerialize["sorXmacIue"] = o.SorXmacIue
	}
	if !openapi.IsNil(o.SorMacIue) {
		toSerialize["sorMacIue"] = o.SorMacIue
	}
	if !openapi.IsNil(o.MeSupportOfSorCmci) {
		toSerialize["meSupportOfSorCmci"] = o.MeSupportOfSorCmci
	}
	if !openapi.IsNil(o.MeSupportOfSorSnpnSi) {
		toSerialize["meSupportOfSorSnpnSi"] = o.MeSupportOfSorSnpnSi
	}
	if !openapi.IsNil(o.MeSupportOfSorSnpnSiLs) {
		toSerialize["meSupportOfSorSnpnSiLs"] = o.MeSupportOfSorSnpnSiLs
	}
	return toSerialize, nil
}

type NullableSorData struct {
	value *SorData
	isSet bool
}

func (v NullableSorData) Get() *SorData {
	return v.value
}

func (v *NullableSorData) Set(val *SorData) {
	v.value = val
	v.isSet = true
}

func (v NullableSorData) IsSet() bool {
	return v.isSet
}

func (v *NullableSorData) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSorData(val *SorData) *NullableSorData {
	return &NullableSorData{value: val, isSet: true}
}

func (v NullableSorData) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSorData) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
