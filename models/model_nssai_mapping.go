// Copyright (C) 2026 Intel Corporation
// SPDX-License-Identifier: Apache-2.0

/*
Namf_Communication

AMF Communication Service.
© 2024, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).
All rights reserved.

API version: 1.3.0-alpha.6

Authors: Aether SD-Core team
Contact: dev@lists.aetherproject.org

Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.
*/

package models

import (
	"encoding/json"

	"github.com/omec-project/openapi/v2"
)

// checks if the NssaiMapping type satisfies the MappedNullable interface at compile time
var _ openapi.MappedNullable = &NssaiMapping{}

// NssaiMapping Represents the mapping between a S-NSSAI in serving PLMN to a S-NSSAI in home PLMN
type NssaiMapping struct {
	MappedSnssai Snssai `json:"mappedSnssai" yaml:"mappedSnssai"`
	HSnssai      Snssai `json:"hSnssai" yaml:"hSnssai"`
}

// NewNssaiMapping instantiates a new NssaiMapping object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewNssaiMapping(mappedSnssai Snssai, hSnssai Snssai) *NssaiMapping {
	this := NssaiMapping{}
	this.MappedSnssai = mappedSnssai
	this.HSnssai = hSnssai
	return &this
}

// NewNssaiMappingWithDefaults instantiates a new NssaiMapping object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewNssaiMappingWithDefaults() *NssaiMapping {
	this := NssaiMapping{}
	return &this
}

// GetMappedSnssai returns the MappedSnssai field value
func (o *NssaiMapping) GetMappedSnssai() Snssai {
	if o == nil {
		var ret Snssai
		return ret
	}

	return o.MappedSnssai
}

// GetMappedSnssaiOk returns a tuple with the MappedSnssai field value
// and a boolean to check if the value has been set.
func (o *NssaiMapping) GetMappedSnssaiOk() (*Snssai, bool) {
	if o == nil {
		return nil, false
	}
	return &o.MappedSnssai, true
}

// SetMappedSnssai sets field value
func (o *NssaiMapping) SetMappedSnssai(v Snssai) {
	o.MappedSnssai = v
}

// GetHSnssai returns the HSnssai field value
func (o *NssaiMapping) GetHSnssai() Snssai {
	if o == nil {
		var ret Snssai
		return ret
	}

	return o.HSnssai
}

// GetHSnssaiOk returns a tuple with the HSnssai field value
// and a boolean to check if the value has been set.
func (o *NssaiMapping) GetHSnssaiOk() (*Snssai, bool) {
	if o == nil {
		return nil, false
	}
	return &o.HSnssai, true
}

// SetHSnssai sets field value
func (o *NssaiMapping) SetHSnssai(v Snssai) {
	o.HSnssai = v
}

func (o NssaiMapping) ToMap() (map[string]any, error) {
	toSerialize := map[string]any{}
	toSerialize["mappedSnssai"] = o.MappedSnssai
	toSerialize["hSnssai"] = o.HSnssai
	return toSerialize, nil
}

type NullableNssaiMapping struct {
	value *NssaiMapping
	isSet bool
}

func (v NullableNssaiMapping) Get() *NssaiMapping {
	return v.value
}

func (v *NullableNssaiMapping) Set(val *NssaiMapping) {
	v.value = val
	v.isSet = true
}

func (v NullableNssaiMapping) IsSet() bool {
	return v.isSet
}

func (v *NullableNssaiMapping) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableNssaiMapping(val *NssaiMapping) *NullableNssaiMapping {
	return &NullableNssaiMapping{value: val, isSet: true}
}

func (v NullableNssaiMapping) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableNssaiMapping) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
