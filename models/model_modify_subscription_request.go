...
func (dst *ModifySubscriptionRequest) UnmarshalJSON(data []byte) error {
	...
	err = openapi.NewStrictDecoder(data).Decode(&dst.ArrayOfAmfUpdateEventOptionItem)
	if err == nil {
	...
	err = openapi.NewStrictDecoder(data).Decode(&dst.ArrayOfAmfUpdateEventSubscriptionItem)
	if err == nil {
	...
	if match > 1 { // more than 1 match
		...
		return openapi.ReportError("data matches more than one schema in oneOf(ModifySubscriptionRequest)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return openapi.ReportError("data failed to match schemas in oneOf(ModifySubscriptionRequest)")
	}
}
...
func (obj *ModifySubscriptionRequest) GetActualInstance() any {
	if obj == nil {
		return nil
	}
...
func (obj ModifySubscriptionRequest) GetActualInstanceValue() any {
	if obj.ArrayOfAmfUpdateEventOptionItem != nil {
		return *obj.ArrayOfAmfUpdateEventOptionItem
	}
