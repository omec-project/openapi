...
func (dst *ComplexQuery) UnmarshalJSON(data []byte) error {
	...
	err = openapi.NewStrictDecoder(data).Decode(&dst.Cnf)
	if err == nil {
	...
	err = openapi.NewStrictDecoder(data).Decode(&dst.Dnf)
	if err == nil {
	...
	if match > 1 { // more than 1 match
		...
		return openapi.ReportError("data matches more than one schema in oneOf(ComplexQuery)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return openapi.ReportError("data failed to match schemas in oneOf(ComplexQuery)")
	}
}
...
func (obj *ComplexQuery) GetActualInstance() any {
	if obj == nil {
		return nil
	}
	if obj.Cnf != nil {
...
func (obj ComplexQuery) GetActualInstanceValue() any {
	if obj.Cnf != nil {
		return *obj.Cnf
	}
