...
func (dst *HssAuthenticationVectors) UnmarshalJSON(data []byte) error {
	...
	err = openapi.NewStrictDecoder(data).Decode(&dst.ArrayOfAvEapAkaPrime)
	if err == nil {
	...
	err = openapi.NewStrictDecoder(data).Decode(&dst.ArrayOfAvEpsAka)
	if err == nil {
	...
	err = openapi.NewStrictDecoder(data).Decode(&dst.ArrayOfAvImsGbaEapAka)
	if err == nil {
	...
	if match > 1 { // more than 1 match
		...
		return openapi.ReportError("data matches more than one schema in oneOf(HssAuthenticationVectors)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return openapi.ReportError("data failed to match schemas in oneOf(HssAuthenticationVectors)")
	}
}
...
func (obj *HssAuthenticationVectors) GetActualInstance() any {
	if obj == nil {
		return nil
	}
...
func (obj HssAuthenticationVectors) GetActualInstanceValue() any {
	if obj.ArrayOfAvEapAkaPrime != nil {
		return *obj.ArrayOfAvEapAkaPrime
	}
