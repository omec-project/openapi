...
func (v *AuthResult) UnmarshalJSON(src []byte) error {
	...
	if slices.Contains(AllowedAuthResultEnumValues, enumTypeValue) {
		*v = enumTypeValue
		return nil
	}

	return openapi.ReportError("%+v is not a valid AuthResult", value)
}
...
func NewAuthResultFromValue(v string) (*AuthResult, error) {
	...
	if ev.IsValid() {
		...
		return nil, openapi.ReportError("invalid value '%v' for AuthResult: valid values are %v", v, AllowedAuthResultEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v AuthResult) IsValid() bool {
	return slices.Contains(AllowedAuthResultEnumValues, v)
}
