// Copyright (C) 2026 Intel Corporation
// SPDX-License-Identifier: Apache-2.0

/*
Nnwdaf_DataManagement

Nnwdaf_DataManagement API Service.
© 2023, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).
All rights reserved.

API version: 1.1.0-alpha.4

Authors: Aether SD-Core team
Contact: dev@lists.aetherproject.org

Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.
*/

package models

import (
	"encoding/json"

	"github.com/omec-project/openapi/v2"
)

// AggregationLevel Represents the aggregation level for processing instructions. Possible values are: - UE: Indicates that the summarized reports shall be provided per UE. - AOI: Indicates that the summarized reports shall be provided per Area of Interest.
type AggregationLevel struct {
	String *string
}

// Unmarshal JSON data into any of the pointers in the struct
func (dst *AggregationLevel) UnmarshalJSON(data []byte) error {
	// try to unmarshal JSON data into String
	err := json.Unmarshal(data, &dst.String)
	if err == nil {
		jsonString, _ := json.Marshal(dst.String)
		if string(jsonString) == "{}" { // empty struct
			dst.String = nil
		} else {
			return nil // data stored in dst.String, return on the first match
		}
	} else {
		dst.String = nil
	}

	return openapi.ReportError("data failed to match schemas in anyOf(AggregationLevel)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src AggregationLevel) MarshalJSON() ([]byte, error) {
	if src.String != nil {
		return json.Marshal(&src.String)
	}

	return nil, nil // no data in anyOf schemas
}

type NullableAggregationLevel struct {
	value *AggregationLevel
	isSet bool
}

func (v NullableAggregationLevel) Get() *AggregationLevel {
	return v.value
}

func (v *NullableAggregationLevel) Set(val *AggregationLevel) {
	v.value = val
	v.isSet = true
}

func (v NullableAggregationLevel) IsSet() bool {
	return v.isSet
}

func (v *NullableAggregationLevel) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAggregationLevel(val *AggregationLevel) *NullableAggregationLevel {
	return &NullableAggregationLevel{value: val, isSet: true}
}

func (v NullableAggregationLevel) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAggregationLevel) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
