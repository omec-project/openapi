// Copyright (C) 2026 Intel Corporation
// SPDX-License-Identifier: Apache-2.0

/*
Npcf_PolicyAuthorization Service API

PCF Policy Authorization Service.
© 2024, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).
All rights reserved.

API version: 1.3.0-alpha.6

Authors: Aether SD-Core team
Contact: dev@lists.aetherproject.org

Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.
*/

package models

import (
	"encoding/json"

	"github.com/omec-project/openapi/v2"
)

// checks if the MediaComponentRm type satisfies the MappedNullable interface at compile time
var _ openapi.MappedNullable = &MediaComponentRm{}

// MediaComponentRm This data type is defined in the same way as the MediaComponent data type, but with the  OpenAPI nullable property set to true.
type MediaComponentRm struct {
	// Contains an AF application identifier.
	AfAppId      *string                        `json:"afAppId,omitempty" yaml:"afAppId,omitempty"`
	AfRoutReq    NullableAfRoutingRequirementRm `json:"afRoutReq" yaml:"afRoutReq,omitempty"`
	AfSfcReq     NullableAfSfcRequirement       `json:"afSfcReq" yaml:"afSfcReq,omitempty"`
	QosReference openapi.NullableString         `json:"qosReference" yaml:"qosReference,omitempty"`
	AltSerReqs   []string                       `json:"altSerReqs,omitempty" yaml:"altSerReqs,omitempty"`
	// Contains removable alternative service requirements that include individual QoS parameter sets.
	AltSerReqsData []AlternativeServiceRequirementsData `json:"altSerReqsData,omitempty" yaml:"altSerReqsData,omitempty"`
	DisUeNotif     *bool                                `json:"disUeNotif,omitempty" yaml:"disUeNotif,omitempty"`
	// Represents the content version of some content.
	ContVer *int32   `json:"contVer,omitempty" yaml:"contVer,omitempty"`
	Codecs  []string `json:"codecs,omitempty" yaml:"codecs,omitempty"`
	// string with format 'float' as defined in OpenAPI with the OpenAPI defined 'nullable: true' property.
	DesMaxLatency openapi.NullableFloat32 `json:"desMaxLatency" yaml:"desMaxLatency,omitempty"`
	// string with format 'float' as defined in OpenAPI with the OpenAPI defined 'nullable: true' property.
	DesMaxLoss openapi.NullableFloat32 `json:"desMaxLoss" yaml:"desMaxLoss,omitempty"`
	FlusId     openapi.NullableString  `json:"flusId" yaml:"flusId,omitempty"`
	FStatus    *FlowStatus             `json:"fStatus,omitempty" yaml:"fStatus,omitempty"`
	// This data type is defined in the same way as the 'BitRate' data type, but with the OpenAPI 'nullable: true' property.
	MarBwDl openapi.NullableString `json:"marBwDl" yaml:"marBwDl,omitempty" validate:"regexp=^\\\\d+(\\\\.\\\\d+)? (bps|Kbps|Mbps|Gbps|Tbps)$"`
	// This data type is defined in the same way as the 'BitRate' data type, but with the OpenAPI 'nullable: true' property.
	MarBwUl openapi.NullableString `json:"marBwUl" yaml:"marBwUl,omitempty" validate:"regexp=^\\\\d+(\\\\.\\\\d+)? (bps|Kbps|Mbps|Gbps|Tbps)$"`
	// This data type is defined in the same way as the 'PacketLossRate' data type, but with the OpenAPI 'nullable: true' property.
	MaxPacketLossRateDl openapi.NullableInt32 `json:"maxPacketLossRateDl" yaml:"maxPacketLossRateDl,omitempty"`
	// This data type is defined in the same way as the 'PacketLossRate' data type, but with the OpenAPI 'nullable: true' property.
	MaxPacketLossRateUl openapi.NullableInt32 `json:"maxPacketLossRateUl" yaml:"maxPacketLossRateUl,omitempty"`
	// This data type is defined in the same way as the 'BitRate' data type, but with the OpenAPI 'nullable: true' property.
	MaxSuppBwDl openapi.NullableString `json:"maxSuppBwDl" yaml:"maxSuppBwDl,omitempty" validate:"regexp=^\\\\d+(\\\\.\\\\d+)? (bps|Kbps|Mbps|Gbps|Tbps)$"`
	// This data type is defined in the same way as the 'BitRate' data type, but with the OpenAPI 'nullable: true' property.
	MaxSuppBwUl openapi.NullableString `json:"maxSuppBwUl" yaml:"maxSuppBwUl,omitempty" validate:"regexp=^\\\\d+(\\\\.\\\\d+)? (bps|Kbps|Mbps|Gbps|Tbps)$"`
	MedCompN    int32                  `json:"medCompN" yaml:"medCompN"`
	// Contains the requested bitrate and filters for the set of service data flows identified by their common flow identifier. The key of the map is the fNum attribute.
	MedSubComps *map[string]MediaSubComponentRm `json:"medSubComps,omitempty" yaml:"medSubComps,omitempty"`
	MedType     *MediaType                      `json:"medType,omitempty" yaml:"medType,omitempty"`
	// This data type is defined in the same way as the 'BitRate' data type, but with the OpenAPI 'nullable: true' property.
	MinDesBwDl openapi.NullableString `json:"minDesBwDl" yaml:"minDesBwDl,omitempty" validate:"regexp=^\\\\d+(\\\\.\\\\d+)? (bps|Kbps|Mbps|Gbps|Tbps)$"`
	// This data type is defined in the same way as the 'BitRate' data type, but with the OpenAPI 'nullable: true' property.
	MinDesBwUl openapi.NullableString `json:"minDesBwUl" yaml:"minDesBwUl,omitempty" validate:"regexp=^\\\\d+(\\\\.\\\\d+)? (bps|Kbps|Mbps|Gbps|Tbps)$"`
	// This data type is defined in the same way as the 'BitRate' data type, but with the OpenAPI 'nullable: true' property.
	MirBwDl openapi.NullableString `json:"mirBwDl" yaml:"mirBwDl,omitempty" validate:"regexp=^\\\\d+(\\\\.\\\\d+)? (bps|Kbps|Mbps|Gbps|Tbps)$"`
	// This data type is defined in the same way as the 'BitRate' data type, but with the OpenAPI 'nullable: true' property.
	MirBwUl        openapi.NullableString     `json:"mirBwUl" yaml:"mirBwUl,omitempty" validate:"regexp=^\\\\d+(\\\\.\\\\d+)? (bps|Kbps|Mbps|Gbps|Tbps)$"`
	PreemptCap     *PreemptionCapabilityRm    `json:"preemptCap,omitempty" yaml:"preemptCap,omitempty"`
	PreemptVuln    *PreemptionVulnerabilityRm `json:"preemptVuln,omitempty" yaml:"preemptVuln,omitempty"`
	PrioSharingInd *PrioritySharingIndicator  `json:"prioSharingInd,omitempty" yaml:"prioSharingInd,omitempty"`
	ResPrio        *ReservPriority            `json:"resPrio,omitempty" yaml:"resPrio,omitempty"`
	// This data type is defined in the same way as the 'BitRate' data type, but with the OpenAPI 'nullable: true' property.
	RrBw openapi.NullableString `json:"rrBw" yaml:"rrBw,omitempty" validate:"regexp=^\\\\d+(\\\\.\\\\d+)? (bps|Kbps|Mbps|Gbps|Tbps)$"`
	// This data type is defined in the same way as the 'BitRate' data type, but with the OpenAPI 'nullable: true' property.
	RsBw openapi.NullableString `json:"rsBw" yaml:"rsBw,omitempty" validate:"regexp=^\\\\d+(\\\\.\\\\d+)? (bps|Kbps|Mbps|Gbps|Tbps)$"`
	// Integer where the allowed values correspond to the value range of an unsigned 32-bit integer with the OpenAPI 'nullable: true' property.
	SharingKeyDl openapi.NullableInt32 `json:"sharingKeyDl" yaml:"sharingKeyDl,omitempty"`
	// Integer where the allowed values correspond to the value range of an unsigned 32-bit integer with the OpenAPI 'nullable: true' property.
	SharingKeyUl openapi.NullableInt32       `json:"sharingKeyUl" yaml:"sharingKeyUl,omitempty"`
	TsnQos       NullableTsnQosContainerRm   `json:"tsnQos" yaml:"tsnQos,omitempty"`
	TscaiInputDl NullableTscaiInputContainer `json:"tscaiInputDl" yaml:"tscaiInputDl,omitempty"`
	TscaiInputUl NullableTscaiInputContainer `json:"tscaiInputUl" yaml:"tscaiInputUl,omitempty"`
	// Unsigned Integer, i.e. only value 0 and integers above 0 are permissible.
	TscaiTimeDom *int32 `json:"tscaiTimeDom,omitempty" yaml:"tscaiTimeDom,omitempty"`
	// Indicates the capability for AF to adjust the burst sending time, when it is supported and set to \"true\". The default value is \"false\" if omitted.
	CapBatAdaptation *bool `json:"capBatAdaptation,omitempty" yaml:"capBatAdaptation,omitempty"`
	// Indicates the service data flow needs to meet the Round-Trip (RT) latency requirement of the service, when it is included and set to \"true\". The default value is \"false\" if omitted.
	RTLatencyInd *bool                   `json:"rTLatencyInd,omitempty" yaml:"rTLatencyInd,omitempty"`
	PduSetQosDl  NullablePduSetQosParaRm `json:"pduSetQosDl" yaml:"pduSetQosDl,omitempty"`
	PduSetQosUl  NullablePduSetQosParaRm `json:"pduSetQosUl" yaml:"pduSetQosUl,omitempty"`
	ProtoDescDl  *ProtocolDescription    `json:"protoDescDl,omitempty" yaml:"protoDescDl,omitempty"`
	ProtoDescUl  *ProtocolDescription    `json:"protoDescUl,omitempty" yaml:"protoDescUl,omitempty"`
	// This data type is defined in the same way as the \"DurationMillisec\" data type, but with the OpenAPI nullable property set to true.
	PeriodUl *int32 `json:"periodUl,omitempty" yaml:"periodUl,omitempty"`
	// This data type is defined in the same way as the \"DurationMillisec\" data type, but with the OpenAPI nullable property set to true.
	PeriodDl *int32                 `json:"periodDl,omitempty" yaml:"periodDl,omitempty"`
	L4sInd   *UplinkDownlinkSupport `json:"l4sInd,omitempty" yaml:"l4sInd,omitempty"`
}

// NewMediaComponentRm instantiates a new MediaComponentRm object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewMediaComponentRm(medCompN int32) *MediaComponentRm {
	this := MediaComponentRm{}
	this.MedCompN = medCompN
	return &this
}

// NewMediaComponentRmWithDefaults instantiates a new MediaComponentRm object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewMediaComponentRmWithDefaults() *MediaComponentRm {
	this := MediaComponentRm{}
	return &this
}

// GetAfAppId returns the AfAppId field value if set, zero value otherwise.
func (o *MediaComponentRm) GetAfAppId() string {
	if o == nil || openapi.IsNil(o.AfAppId) {
		var ret string
		return ret
	}
	return *o.AfAppId
}

// GetAfAppIdOk returns a tuple with the AfAppId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetAfAppIdOk() (*string, bool) {
	if o == nil || openapi.IsNil(o.AfAppId) {
		return nil, false
	}
	return o.AfAppId, true
}

// HasAfAppId returns a boolean if a field has been set.
func (o *MediaComponentRm) HasAfAppId() bool {
	if o != nil && !openapi.IsNil(o.AfAppId) {
		return true
	}

	return false
}

// SetAfAppId gets a reference to the given string and assigns it to the AfAppId field.
func (o *MediaComponentRm) SetAfAppId(v string) {
	o.AfAppId = &v
}

// GetAfRoutReq returns the AfRoutReq field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetAfRoutReq() AfRoutingRequirementRm {
	if o == nil || openapi.IsNil(o.AfRoutReq.Get()) {
		var ret AfRoutingRequirementRm
		return ret
	}
	return *o.AfRoutReq.Get()
}

// GetAfRoutReqOk returns a tuple with the AfRoutReq field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetAfRoutReqOk() (*AfRoutingRequirementRm, bool) {
	if o == nil {
		return nil, false
	}
	return o.AfRoutReq.Get(), o.AfRoutReq.IsSet()
}

// HasAfRoutReq returns a boolean if a field has been set.
func (o *MediaComponentRm) HasAfRoutReq() bool {
	if o != nil && o.AfRoutReq.IsSet() {
		return true
	}

	return false
}

// SetAfRoutReq gets a reference to the given NullableAfRoutingRequirementRm and assigns it to the AfRoutReq field.
func (o *MediaComponentRm) SetAfRoutReq(v AfRoutingRequirementRm) {
	o.AfRoutReq.Set(&v)
}

// SetAfRoutReqNil sets the value for AfRoutReq to be an explicit nil
func (o *MediaComponentRm) SetAfRoutReqNil() {
	o.AfRoutReq.Set(nil)
}

// UnsetAfRoutReq ensures that no value is present for AfRoutReq, not even an explicit nil
func (o *MediaComponentRm) UnsetAfRoutReq() {
	o.AfRoutReq.Unset()
}

// GetAfSfcReq returns the AfSfcReq field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetAfSfcReq() AfSfcRequirement {
	if o == nil || openapi.IsNil(o.AfSfcReq.Get()) {
		var ret AfSfcRequirement
		return ret
	}
	return *o.AfSfcReq.Get()
}

// GetAfSfcReqOk returns a tuple with the AfSfcReq field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetAfSfcReqOk() (*AfSfcRequirement, bool) {
	if o == nil {
		return nil, false
	}
	return o.AfSfcReq.Get(), o.AfSfcReq.IsSet()
}

// HasAfSfcReq returns a boolean if a field has been set.
func (o *MediaComponentRm) HasAfSfcReq() bool {
	if o != nil && o.AfSfcReq.IsSet() {
		return true
	}

	return false
}

// SetAfSfcReq gets a reference to the given NullableAfSfcRequirement and assigns it to the AfSfcReq field.
func (o *MediaComponentRm) SetAfSfcReq(v AfSfcRequirement) {
	o.AfSfcReq.Set(&v)
}

// SetAfSfcReqNil sets the value for AfSfcReq to be an explicit nil
func (o *MediaComponentRm) SetAfSfcReqNil() {
	o.AfSfcReq.Set(nil)
}

// UnsetAfSfcReq ensures that no value is present for AfSfcReq, not even an explicit nil
func (o *MediaComponentRm) UnsetAfSfcReq() {
	o.AfSfcReq.Unset()
}

// GetQosReference returns the QosReference field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetQosReference() string {
	if o == nil || openapi.IsNil(o.QosReference.Get()) {
		var ret string
		return ret
	}
	return *o.QosReference.Get()
}

// GetQosReferenceOk returns a tuple with the QosReference field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetQosReferenceOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.QosReference.Get(), o.QosReference.IsSet()
}

// HasQosReference returns a boolean if a field has been set.
func (o *MediaComponentRm) HasQosReference() bool {
	if o != nil && o.QosReference.IsSet() {
		return true
	}

	return false
}

// SetQosReference gets a reference to the given openapi.NullableString and assigns it to the QosReference field.
func (o *MediaComponentRm) SetQosReference(v string) {
	o.QosReference.Set(&v)
}

// SetQosReferenceNil sets the value for QosReference to be an explicit nil
func (o *MediaComponentRm) SetQosReferenceNil() {
	o.QosReference.Set(nil)
}

// UnsetQosReference ensures that no value is present for QosReference, not even an explicit nil
func (o *MediaComponentRm) UnsetQosReference() {
	o.QosReference.Unset()
}

// GetAltSerReqs returns the AltSerReqs field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetAltSerReqs() []string {
	if o == nil {
		var ret []string
		return ret
	}
	return o.AltSerReqs
}

// GetAltSerReqsOk returns a tuple with the AltSerReqs field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetAltSerReqsOk() ([]string, bool) {
	if o == nil || openapi.IsNil(o.AltSerReqs) {
		return nil, false
	}
	return o.AltSerReqs, true
}

// HasAltSerReqs returns a boolean if a field has been set.
func (o *MediaComponentRm) HasAltSerReqs() bool {
	if o != nil && !openapi.IsNil(o.AltSerReqs) {
		return true
	}

	return false
}

// SetAltSerReqs gets a reference to the given []string and assigns it to the AltSerReqs field.
func (o *MediaComponentRm) SetAltSerReqs(v []string) {
	o.AltSerReqs = v
}

// GetAltSerReqsData returns the AltSerReqsData field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetAltSerReqsData() []AlternativeServiceRequirementsData {
	if o == nil {
		var ret []AlternativeServiceRequirementsData
		return ret
	}
	return o.AltSerReqsData
}

// GetAltSerReqsDataOk returns a tuple with the AltSerReqsData field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetAltSerReqsDataOk() ([]AlternativeServiceRequirementsData, bool) {
	if o == nil || openapi.IsNil(o.AltSerReqsData) {
		return nil, false
	}
	return o.AltSerReqsData, true
}

// HasAltSerReqsData returns a boolean if a field has been set.
func (o *MediaComponentRm) HasAltSerReqsData() bool {
	if o != nil && !openapi.IsNil(o.AltSerReqsData) {
		return true
	}

	return false
}

// SetAltSerReqsData gets a reference to the given []AlternativeServiceRequirementsData and assigns it to the AltSerReqsData field.
func (o *MediaComponentRm) SetAltSerReqsData(v []AlternativeServiceRequirementsData) {
	o.AltSerReqsData = v
}

// GetDisUeNotif returns the DisUeNotif field value if set, zero value otherwise.
func (o *MediaComponentRm) GetDisUeNotif() bool {
	if o == nil || openapi.IsNil(o.DisUeNotif) {
		var ret bool
		return ret
	}
	return *o.DisUeNotif
}

// GetDisUeNotifOk returns a tuple with the DisUeNotif field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetDisUeNotifOk() (*bool, bool) {
	if o == nil || openapi.IsNil(o.DisUeNotif) {
		return nil, false
	}
	return o.DisUeNotif, true
}

// HasDisUeNotif returns a boolean if a field has been set.
func (o *MediaComponentRm) HasDisUeNotif() bool {
	if o != nil && !openapi.IsNil(o.DisUeNotif) {
		return true
	}

	return false
}

// SetDisUeNotif gets a reference to the given bool and assigns it to the DisUeNotif field.
func (o *MediaComponentRm) SetDisUeNotif(v bool) {
	o.DisUeNotif = &v
}

// GetContVer returns the ContVer field value if set, zero value otherwise.
func (o *MediaComponentRm) GetContVer() int32 {
	if o == nil || openapi.IsNil(o.ContVer) {
		var ret int32
		return ret
	}
	return *o.ContVer
}

// GetContVerOk returns a tuple with the ContVer field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetContVerOk() (*int32, bool) {
	if o == nil || openapi.IsNil(o.ContVer) {
		return nil, false
	}
	return o.ContVer, true
}

// HasContVer returns a boolean if a field has been set.
func (o *MediaComponentRm) HasContVer() bool {
	if o != nil && !openapi.IsNil(o.ContVer) {
		return true
	}

	return false
}

// SetContVer gets a reference to the given int32 and assigns it to the ContVer field.
func (o *MediaComponentRm) SetContVer(v int32) {
	o.ContVer = &v
}

// GetCodecs returns the Codecs field value if set, zero value otherwise.
func (o *MediaComponentRm) GetCodecs() []string {
	if o == nil || openapi.IsNil(o.Codecs) {
		var ret []string
		return ret
	}
	return o.Codecs
}

// GetCodecsOk returns a tuple with the Codecs field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetCodecsOk() ([]string, bool) {
	if o == nil || openapi.IsNil(o.Codecs) {
		return nil, false
	}
	return o.Codecs, true
}

// HasCodecs returns a boolean if a field has been set.
func (o *MediaComponentRm) HasCodecs() bool {
	if o != nil && !openapi.IsNil(o.Codecs) {
		return true
	}

	return false
}

// SetCodecs gets a reference to the given []string and assigns it to the Codecs field.
func (o *MediaComponentRm) SetCodecs(v []string) {
	o.Codecs = v
}

// GetDesMaxLatency returns the DesMaxLatency field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetDesMaxLatency() float32 {
	if o == nil || openapi.IsNil(o.DesMaxLatency.Get()) {
		var ret float32
		return ret
	}
	return *o.DesMaxLatency.Get()
}

// GetDesMaxLatencyOk returns a tuple with the DesMaxLatency field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetDesMaxLatencyOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return o.DesMaxLatency.Get(), o.DesMaxLatency.IsSet()
}

// HasDesMaxLatency returns a boolean if a field has been set.
func (o *MediaComponentRm) HasDesMaxLatency() bool {
	if o != nil && o.DesMaxLatency.IsSet() {
		return true
	}

	return false
}

// SetDesMaxLatency gets a reference to the given openapi.NullableFloat32 and assigns it to the DesMaxLatency field.
func (o *MediaComponentRm) SetDesMaxLatency(v float32) {
	o.DesMaxLatency.Set(&v)
}

// SetDesMaxLatencyNil sets the value for DesMaxLatency to be an explicit nil
func (o *MediaComponentRm) SetDesMaxLatencyNil() {
	o.DesMaxLatency.Set(nil)
}

// UnsetDesMaxLatency ensures that no value is present for DesMaxLatency, not even an explicit nil
func (o *MediaComponentRm) UnsetDesMaxLatency() {
	o.DesMaxLatency.Unset()
}

// GetDesMaxLoss returns the DesMaxLoss field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetDesMaxLoss() float32 {
	if o == nil || openapi.IsNil(o.DesMaxLoss.Get()) {
		var ret float32
		return ret
	}
	return *o.DesMaxLoss.Get()
}

// GetDesMaxLossOk returns a tuple with the DesMaxLoss field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetDesMaxLossOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return o.DesMaxLoss.Get(), o.DesMaxLoss.IsSet()
}

// HasDesMaxLoss returns a boolean if a field has been set.
func (o *MediaComponentRm) HasDesMaxLoss() bool {
	if o != nil && o.DesMaxLoss.IsSet() {
		return true
	}

	return false
}

// SetDesMaxLoss gets a reference to the given openapi.NullableFloat32 and assigns it to the DesMaxLoss field.
func (o *MediaComponentRm) SetDesMaxLoss(v float32) {
	o.DesMaxLoss.Set(&v)
}

// SetDesMaxLossNil sets the value for DesMaxLoss to be an explicit nil
func (o *MediaComponentRm) SetDesMaxLossNil() {
	o.DesMaxLoss.Set(nil)
}

// UnsetDesMaxLoss ensures that no value is present for DesMaxLoss, not even an explicit nil
func (o *MediaComponentRm) UnsetDesMaxLoss() {
	o.DesMaxLoss.Unset()
}

// GetFlusId returns the FlusId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetFlusId() string {
	if o == nil || openapi.IsNil(o.FlusId.Get()) {
		var ret string
		return ret
	}
	return *o.FlusId.Get()
}

// GetFlusIdOk returns a tuple with the FlusId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetFlusIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.FlusId.Get(), o.FlusId.IsSet()
}

// HasFlusId returns a boolean if a field has been set.
func (o *MediaComponentRm) HasFlusId() bool {
	if o != nil && o.FlusId.IsSet() {
		return true
	}

	return false
}

// SetFlusId gets a reference to the given openapi.NullableString and assigns it to the FlusId field.
func (o *MediaComponentRm) SetFlusId(v string) {
	o.FlusId.Set(&v)
}

// SetFlusIdNil sets the value for FlusId to be an explicit nil
func (o *MediaComponentRm) SetFlusIdNil() {
	o.FlusId.Set(nil)
}

// UnsetFlusId ensures that no value is present for FlusId, not even an explicit nil
func (o *MediaComponentRm) UnsetFlusId() {
	o.FlusId.Unset()
}

// GetFStatus returns the FStatus field value if set, zero value otherwise.
func (o *MediaComponentRm) GetFStatus() FlowStatus {
	if o == nil || openapi.IsNil(o.FStatus) {
		var ret FlowStatus
		return ret
	}
	return *o.FStatus
}

// GetFStatusOk returns a tuple with the FStatus field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetFStatusOk() (*FlowStatus, bool) {
	if o == nil || openapi.IsNil(o.FStatus) {
		return nil, false
	}
	return o.FStatus, true
}

// HasFStatus returns a boolean if a field has been set.
func (o *MediaComponentRm) HasFStatus() bool {
	if o != nil && !openapi.IsNil(o.FStatus) {
		return true
	}

	return false
}

// SetFStatus gets a reference to the given FlowStatus and assigns it to the FStatus field.
func (o *MediaComponentRm) SetFStatus(v FlowStatus) {
	o.FStatus = &v
}

// GetMarBwDl returns the MarBwDl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetMarBwDl() string {
	if o == nil || openapi.IsNil(o.MarBwDl.Get()) {
		var ret string
		return ret
	}
	return *o.MarBwDl.Get()
}

// GetMarBwDlOk returns a tuple with the MarBwDl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetMarBwDlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.MarBwDl.Get(), o.MarBwDl.IsSet()
}

// HasMarBwDl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasMarBwDl() bool {
	if o != nil && o.MarBwDl.IsSet() {
		return true
	}

	return false
}

// SetMarBwDl gets a reference to the given openapi.NullableString and assigns it to the MarBwDl field.
func (o *MediaComponentRm) SetMarBwDl(v string) {
	o.MarBwDl.Set(&v)
}

// SetMarBwDlNil sets the value for MarBwDl to be an explicit nil
func (o *MediaComponentRm) SetMarBwDlNil() {
	o.MarBwDl.Set(nil)
}

// UnsetMarBwDl ensures that no value is present for MarBwDl, not even an explicit nil
func (o *MediaComponentRm) UnsetMarBwDl() {
	o.MarBwDl.Unset()
}

// GetMarBwUl returns the MarBwUl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetMarBwUl() string {
	if o == nil || openapi.IsNil(o.MarBwUl.Get()) {
		var ret string
		return ret
	}
	return *o.MarBwUl.Get()
}

// GetMarBwUlOk returns a tuple with the MarBwUl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetMarBwUlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.MarBwUl.Get(), o.MarBwUl.IsSet()
}

// HasMarBwUl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasMarBwUl() bool {
	if o != nil && o.MarBwUl.IsSet() {
		return true
	}

	return false
}

// SetMarBwUl gets a reference to the given openapi.NullableString and assigns it to the MarBwUl field.
func (o *MediaComponentRm) SetMarBwUl(v string) {
	o.MarBwUl.Set(&v)
}

// SetMarBwUlNil sets the value for MarBwUl to be an explicit nil
func (o *MediaComponentRm) SetMarBwUlNil() {
	o.MarBwUl.Set(nil)
}

// UnsetMarBwUl ensures that no value is present for MarBwUl, not even an explicit nil
func (o *MediaComponentRm) UnsetMarBwUl() {
	o.MarBwUl.Unset()
}

// GetMaxPacketLossRateDl returns the MaxPacketLossRateDl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetMaxPacketLossRateDl() int32 {
	if o == nil || openapi.IsNil(o.MaxPacketLossRateDl.Get()) {
		var ret int32
		return ret
	}
	return *o.MaxPacketLossRateDl.Get()
}

// GetMaxPacketLossRateDlOk returns a tuple with the MaxPacketLossRateDl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetMaxPacketLossRateDlOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.MaxPacketLossRateDl.Get(), o.MaxPacketLossRateDl.IsSet()
}

// HasMaxPacketLossRateDl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasMaxPacketLossRateDl() bool {
	if o != nil && o.MaxPacketLossRateDl.IsSet() {
		return true
	}

	return false
}

// SetMaxPacketLossRateDl gets a reference to the given openapi.NullableInt32 and assigns it to the MaxPacketLossRateDl field.
func (o *MediaComponentRm) SetMaxPacketLossRateDl(v int32) {
	o.MaxPacketLossRateDl.Set(&v)
}

// SetMaxPacketLossRateDlNil sets the value for MaxPacketLossRateDl to be an explicit nil
func (o *MediaComponentRm) SetMaxPacketLossRateDlNil() {
	o.MaxPacketLossRateDl.Set(nil)
}

// UnsetMaxPacketLossRateDl ensures that no value is present for MaxPacketLossRateDl, not even an explicit nil
func (o *MediaComponentRm) UnsetMaxPacketLossRateDl() {
	o.MaxPacketLossRateDl.Unset()
}

// GetMaxPacketLossRateUl returns the MaxPacketLossRateUl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetMaxPacketLossRateUl() int32 {
	if o == nil || openapi.IsNil(o.MaxPacketLossRateUl.Get()) {
		var ret int32
		return ret
	}
	return *o.MaxPacketLossRateUl.Get()
}

// GetMaxPacketLossRateUlOk returns a tuple with the MaxPacketLossRateUl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetMaxPacketLossRateUlOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.MaxPacketLossRateUl.Get(), o.MaxPacketLossRateUl.IsSet()
}

// HasMaxPacketLossRateUl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasMaxPacketLossRateUl() bool {
	if o != nil && o.MaxPacketLossRateUl.IsSet() {
		return true
	}

	return false
}

// SetMaxPacketLossRateUl gets a reference to the given openapi.NullableInt32 and assigns it to the MaxPacketLossRateUl field.
func (o *MediaComponentRm) SetMaxPacketLossRateUl(v int32) {
	o.MaxPacketLossRateUl.Set(&v)
}

// SetMaxPacketLossRateUlNil sets the value for MaxPacketLossRateUl to be an explicit nil
func (o *MediaComponentRm) SetMaxPacketLossRateUlNil() {
	o.MaxPacketLossRateUl.Set(nil)
}

// UnsetMaxPacketLossRateUl ensures that no value is present for MaxPacketLossRateUl, not even an explicit nil
func (o *MediaComponentRm) UnsetMaxPacketLossRateUl() {
	o.MaxPacketLossRateUl.Unset()
}

// GetMaxSuppBwDl returns the MaxSuppBwDl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetMaxSuppBwDl() string {
	if o == nil || openapi.IsNil(o.MaxSuppBwDl.Get()) {
		var ret string
		return ret
	}
	return *o.MaxSuppBwDl.Get()
}

// GetMaxSuppBwDlOk returns a tuple with the MaxSuppBwDl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetMaxSuppBwDlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.MaxSuppBwDl.Get(), o.MaxSuppBwDl.IsSet()
}

// HasMaxSuppBwDl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasMaxSuppBwDl() bool {
	if o != nil && o.MaxSuppBwDl.IsSet() {
		return true
	}

	return false
}

// SetMaxSuppBwDl gets a reference to the given openapi.NullableString and assigns it to the MaxSuppBwDl field.
func (o *MediaComponentRm) SetMaxSuppBwDl(v string) {
	o.MaxSuppBwDl.Set(&v)
}

// SetMaxSuppBwDlNil sets the value for MaxSuppBwDl to be an explicit nil
func (o *MediaComponentRm) SetMaxSuppBwDlNil() {
	o.MaxSuppBwDl.Set(nil)
}

// UnsetMaxSuppBwDl ensures that no value is present for MaxSuppBwDl, not even an explicit nil
func (o *MediaComponentRm) UnsetMaxSuppBwDl() {
	o.MaxSuppBwDl.Unset()
}

// GetMaxSuppBwUl returns the MaxSuppBwUl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetMaxSuppBwUl() string {
	if o == nil || openapi.IsNil(o.MaxSuppBwUl.Get()) {
		var ret string
		return ret
	}
	return *o.MaxSuppBwUl.Get()
}

// GetMaxSuppBwUlOk returns a tuple with the MaxSuppBwUl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetMaxSuppBwUlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.MaxSuppBwUl.Get(), o.MaxSuppBwUl.IsSet()
}

// HasMaxSuppBwUl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasMaxSuppBwUl() bool {
	if o != nil && o.MaxSuppBwUl.IsSet() {
		return true
	}

	return false
}

// SetMaxSuppBwUl gets a reference to the given openapi.NullableString and assigns it to the MaxSuppBwUl field.
func (o *MediaComponentRm) SetMaxSuppBwUl(v string) {
	o.MaxSuppBwUl.Set(&v)
}

// SetMaxSuppBwUlNil sets the value for MaxSuppBwUl to be an explicit nil
func (o *MediaComponentRm) SetMaxSuppBwUlNil() {
	o.MaxSuppBwUl.Set(nil)
}

// UnsetMaxSuppBwUl ensures that no value is present for MaxSuppBwUl, not even an explicit nil
func (o *MediaComponentRm) UnsetMaxSuppBwUl() {
	o.MaxSuppBwUl.Unset()
}

// GetMedCompN returns the MedCompN field value
func (o *MediaComponentRm) GetMedCompN() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.MedCompN
}

// GetMedCompNOk returns a tuple with the MedCompN field value
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetMedCompNOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.MedCompN, true
}

// SetMedCompN sets field value
func (o *MediaComponentRm) SetMedCompN(v int32) {
	o.MedCompN = v
}

// GetMedSubComps returns the MedSubComps field value if set, zero value otherwise.
func (o *MediaComponentRm) GetMedSubComps() map[string]MediaSubComponentRm {
	if o == nil || openapi.IsNil(o.MedSubComps) {
		var ret map[string]MediaSubComponentRm
		return ret
	}
	return *o.MedSubComps
}

// GetMedSubCompsOk returns a tuple with the MedSubComps field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetMedSubCompsOk() (*map[string]MediaSubComponentRm, bool) {
	if o == nil || openapi.IsNil(o.MedSubComps) {
		return nil, false
	}
	return o.MedSubComps, true
}

// HasMedSubComps returns a boolean if a field has been set.
func (o *MediaComponentRm) HasMedSubComps() bool {
	if o != nil && !openapi.IsNil(o.MedSubComps) {
		return true
	}

	return false
}

// SetMedSubComps gets a reference to the given map[string]MediaSubComponentRm and assigns it to the MedSubComps field.
func (o *MediaComponentRm) SetMedSubComps(v map[string]MediaSubComponentRm) {
	o.MedSubComps = &v
}

// GetMedType returns the MedType field value if set, zero value otherwise.
func (o *MediaComponentRm) GetMedType() MediaType {
	if o == nil || openapi.IsNil(o.MedType) {
		var ret MediaType
		return ret
	}
	return *o.MedType
}

// GetMedTypeOk returns a tuple with the MedType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetMedTypeOk() (*MediaType, bool) {
	if o == nil || openapi.IsNil(o.MedType) {
		return nil, false
	}
	return o.MedType, true
}

// HasMedType returns a boolean if a field has been set.
func (o *MediaComponentRm) HasMedType() bool {
	if o != nil && !openapi.IsNil(o.MedType) {
		return true
	}

	return false
}

// SetMedType gets a reference to the given MediaType and assigns it to the MedType field.
func (o *MediaComponentRm) SetMedType(v MediaType) {
	o.MedType = &v
}

// GetMinDesBwDl returns the MinDesBwDl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetMinDesBwDl() string {
	if o == nil || openapi.IsNil(o.MinDesBwDl.Get()) {
		var ret string
		return ret
	}
	return *o.MinDesBwDl.Get()
}

// GetMinDesBwDlOk returns a tuple with the MinDesBwDl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetMinDesBwDlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.MinDesBwDl.Get(), o.MinDesBwDl.IsSet()
}

// HasMinDesBwDl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasMinDesBwDl() bool {
	if o != nil && o.MinDesBwDl.IsSet() {
		return true
	}

	return false
}

// SetMinDesBwDl gets a reference to the given openapi.NullableString and assigns it to the MinDesBwDl field.
func (o *MediaComponentRm) SetMinDesBwDl(v string) {
	o.MinDesBwDl.Set(&v)
}

// SetMinDesBwDlNil sets the value for MinDesBwDl to be an explicit nil
func (o *MediaComponentRm) SetMinDesBwDlNil() {
	o.MinDesBwDl.Set(nil)
}

// UnsetMinDesBwDl ensures that no value is present for MinDesBwDl, not even an explicit nil
func (o *MediaComponentRm) UnsetMinDesBwDl() {
	o.MinDesBwDl.Unset()
}

// GetMinDesBwUl returns the MinDesBwUl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetMinDesBwUl() string {
	if o == nil || openapi.IsNil(o.MinDesBwUl.Get()) {
		var ret string
		return ret
	}
	return *o.MinDesBwUl.Get()
}

// GetMinDesBwUlOk returns a tuple with the MinDesBwUl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetMinDesBwUlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.MinDesBwUl.Get(), o.MinDesBwUl.IsSet()
}

// HasMinDesBwUl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasMinDesBwUl() bool {
	if o != nil && o.MinDesBwUl.IsSet() {
		return true
	}

	return false
}

// SetMinDesBwUl gets a reference to the given openapi.NullableString and assigns it to the MinDesBwUl field.
func (o *MediaComponentRm) SetMinDesBwUl(v string) {
	o.MinDesBwUl.Set(&v)
}

// SetMinDesBwUlNil sets the value for MinDesBwUl to be an explicit nil
func (o *MediaComponentRm) SetMinDesBwUlNil() {
	o.MinDesBwUl.Set(nil)
}

// UnsetMinDesBwUl ensures that no value is present for MinDesBwUl, not even an explicit nil
func (o *MediaComponentRm) UnsetMinDesBwUl() {
	o.MinDesBwUl.Unset()
}

// GetMirBwDl returns the MirBwDl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetMirBwDl() string {
	if o == nil || openapi.IsNil(o.MirBwDl.Get()) {
		var ret string
		return ret
	}
	return *o.MirBwDl.Get()
}

// GetMirBwDlOk returns a tuple with the MirBwDl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetMirBwDlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.MirBwDl.Get(), o.MirBwDl.IsSet()
}

// HasMirBwDl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasMirBwDl() bool {
	if o != nil && o.MirBwDl.IsSet() {
		return true
	}

	return false
}

// SetMirBwDl gets a reference to the given openapi.NullableString and assigns it to the MirBwDl field.
func (o *MediaComponentRm) SetMirBwDl(v string) {
	o.MirBwDl.Set(&v)
}

// SetMirBwDlNil sets the value for MirBwDl to be an explicit nil
func (o *MediaComponentRm) SetMirBwDlNil() {
	o.MirBwDl.Set(nil)
}

// UnsetMirBwDl ensures that no value is present for MirBwDl, not even an explicit nil
func (o *MediaComponentRm) UnsetMirBwDl() {
	o.MirBwDl.Unset()
}

// GetMirBwUl returns the MirBwUl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetMirBwUl() string {
	if o == nil || openapi.IsNil(o.MirBwUl.Get()) {
		var ret string
		return ret
	}
	return *o.MirBwUl.Get()
}

// GetMirBwUlOk returns a tuple with the MirBwUl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetMirBwUlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.MirBwUl.Get(), o.MirBwUl.IsSet()
}

// HasMirBwUl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasMirBwUl() bool {
	if o != nil && o.MirBwUl.IsSet() {
		return true
	}

	return false
}

// SetMirBwUl gets a reference to the given openapi.NullableString and assigns it to the MirBwUl field.
func (o *MediaComponentRm) SetMirBwUl(v string) {
	o.MirBwUl.Set(&v)
}

// SetMirBwUlNil sets the value for MirBwUl to be an explicit nil
func (o *MediaComponentRm) SetMirBwUlNil() {
	o.MirBwUl.Set(nil)
}

// UnsetMirBwUl ensures that no value is present for MirBwUl, not even an explicit nil
func (o *MediaComponentRm) UnsetMirBwUl() {
	o.MirBwUl.Unset()
}

// GetPreemptCap returns the PreemptCap field value if set, zero value otherwise.
func (o *MediaComponentRm) GetPreemptCap() PreemptionCapabilityRm {
	if o == nil || openapi.IsNil(o.PreemptCap) {
		var ret PreemptionCapabilityRm
		return ret
	}
	return *o.PreemptCap
}

// GetPreemptCapOk returns a tuple with the PreemptCap field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetPreemptCapOk() (*PreemptionCapabilityRm, bool) {
	if o == nil || openapi.IsNil(o.PreemptCap) {
		return nil, false
	}
	return o.PreemptCap, true
}

// HasPreemptCap returns a boolean if a field has been set.
func (o *MediaComponentRm) HasPreemptCap() bool {
	if o != nil && !openapi.IsNil(o.PreemptCap) {
		return true
	}

	return false
}

// SetPreemptCap gets a reference to the given PreemptionCapabilityRm and assigns it to the PreemptCap field.
func (o *MediaComponentRm) SetPreemptCap(v PreemptionCapabilityRm) {
	o.PreemptCap = &v
}

// GetPreemptVuln returns the PreemptVuln field value if set, zero value otherwise.
func (o *MediaComponentRm) GetPreemptVuln() PreemptionVulnerabilityRm {
	if o == nil || openapi.IsNil(o.PreemptVuln) {
		var ret PreemptionVulnerabilityRm
		return ret
	}
	return *o.PreemptVuln
}

// GetPreemptVulnOk returns a tuple with the PreemptVuln field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetPreemptVulnOk() (*PreemptionVulnerabilityRm, bool) {
	if o == nil || openapi.IsNil(o.PreemptVuln) {
		return nil, false
	}
	return o.PreemptVuln, true
}

// HasPreemptVuln returns a boolean if a field has been set.
func (o *MediaComponentRm) HasPreemptVuln() bool {
	if o != nil && !openapi.IsNil(o.PreemptVuln) {
		return true
	}

	return false
}

// SetPreemptVuln gets a reference to the given PreemptionVulnerabilityRm and assigns it to the PreemptVuln field.
func (o *MediaComponentRm) SetPreemptVuln(v PreemptionVulnerabilityRm) {
	o.PreemptVuln = &v
}

// GetPrioSharingInd returns the PrioSharingInd field value if set, zero value otherwise.
func (o *MediaComponentRm) GetPrioSharingInd() PrioritySharingIndicator {
	if o == nil || openapi.IsNil(o.PrioSharingInd) {
		var ret PrioritySharingIndicator
		return ret
	}
	return *o.PrioSharingInd
}

// GetPrioSharingIndOk returns a tuple with the PrioSharingInd field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetPrioSharingIndOk() (*PrioritySharingIndicator, bool) {
	if o == nil || openapi.IsNil(o.PrioSharingInd) {
		return nil, false
	}
	return o.PrioSharingInd, true
}

// HasPrioSharingInd returns a boolean if a field has been set.
func (o *MediaComponentRm) HasPrioSharingInd() bool {
	if o != nil && !openapi.IsNil(o.PrioSharingInd) {
		return true
	}

	return false
}

// SetPrioSharingInd gets a reference to the given PrioritySharingIndicator and assigns it to the PrioSharingInd field.
func (o *MediaComponentRm) SetPrioSharingInd(v PrioritySharingIndicator) {
	o.PrioSharingInd = &v
}

// GetResPrio returns the ResPrio field value if set, zero value otherwise.
func (o *MediaComponentRm) GetResPrio() ReservPriority {
	if o == nil || openapi.IsNil(o.ResPrio) {
		var ret ReservPriority
		return ret
	}
	return *o.ResPrio
}

// GetResPrioOk returns a tuple with the ResPrio field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetResPrioOk() (*ReservPriority, bool) {
	if o == nil || openapi.IsNil(o.ResPrio) {
		return nil, false
	}
	return o.ResPrio, true
}

// HasResPrio returns a boolean if a field has been set.
func (o *MediaComponentRm) HasResPrio() bool {
	if o != nil && !openapi.IsNil(o.ResPrio) {
		return true
	}

	return false
}

// SetResPrio gets a reference to the given ReservPriority and assigns it to the ResPrio field.
func (o *MediaComponentRm) SetResPrio(v ReservPriority) {
	o.ResPrio = &v
}

// GetRrBw returns the RrBw field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetRrBw() string {
	if o == nil || openapi.IsNil(o.RrBw.Get()) {
		var ret string
		return ret
	}
	return *o.RrBw.Get()
}

// GetRrBwOk returns a tuple with the RrBw field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetRrBwOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.RrBw.Get(), o.RrBw.IsSet()
}

// HasRrBw returns a boolean if a field has been set.
func (o *MediaComponentRm) HasRrBw() bool {
	if o != nil && o.RrBw.IsSet() {
		return true
	}

	return false
}

// SetRrBw gets a reference to the given openapi.NullableString and assigns it to the RrBw field.
func (o *MediaComponentRm) SetRrBw(v string) {
	o.RrBw.Set(&v)
}

// SetRrBwNil sets the value for RrBw to be an explicit nil
func (o *MediaComponentRm) SetRrBwNil() {
	o.RrBw.Set(nil)
}

// UnsetRrBw ensures that no value is present for RrBw, not even an explicit nil
func (o *MediaComponentRm) UnsetRrBw() {
	o.RrBw.Unset()
}

// GetRsBw returns the RsBw field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetRsBw() string {
	if o == nil || openapi.IsNil(o.RsBw.Get()) {
		var ret string
		return ret
	}
	return *o.RsBw.Get()
}

// GetRsBwOk returns a tuple with the RsBw field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetRsBwOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.RsBw.Get(), o.RsBw.IsSet()
}

// HasRsBw returns a boolean if a field has been set.
func (o *MediaComponentRm) HasRsBw() bool {
	if o != nil && o.RsBw.IsSet() {
		return true
	}

	return false
}

// SetRsBw gets a reference to the given openapi.NullableString and assigns it to the RsBw field.
func (o *MediaComponentRm) SetRsBw(v string) {
	o.RsBw.Set(&v)
}

// SetRsBwNil sets the value for RsBw to be an explicit nil
func (o *MediaComponentRm) SetRsBwNil() {
	o.RsBw.Set(nil)
}

// UnsetRsBw ensures that no value is present for RsBw, not even an explicit nil
func (o *MediaComponentRm) UnsetRsBw() {
	o.RsBw.Unset()
}

// GetSharingKeyDl returns the SharingKeyDl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetSharingKeyDl() int32 {
	if o == nil || openapi.IsNil(o.SharingKeyDl.Get()) {
		var ret int32
		return ret
	}
	return *o.SharingKeyDl.Get()
}

// GetSharingKeyDlOk returns a tuple with the SharingKeyDl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetSharingKeyDlOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.SharingKeyDl.Get(), o.SharingKeyDl.IsSet()
}

// HasSharingKeyDl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasSharingKeyDl() bool {
	if o != nil && o.SharingKeyDl.IsSet() {
		return true
	}

	return false
}

// SetSharingKeyDl gets a reference to the given openapi.NullableInt32 and assigns it to the SharingKeyDl field.
func (o *MediaComponentRm) SetSharingKeyDl(v int32) {
	o.SharingKeyDl.Set(&v)
}

// SetSharingKeyDlNil sets the value for SharingKeyDl to be an explicit nil
func (o *MediaComponentRm) SetSharingKeyDlNil() {
	o.SharingKeyDl.Set(nil)
}

// UnsetSharingKeyDl ensures that no value is present for SharingKeyDl, not even an explicit nil
func (o *MediaComponentRm) UnsetSharingKeyDl() {
	o.SharingKeyDl.Unset()
}

// GetSharingKeyUl returns the SharingKeyUl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetSharingKeyUl() int32 {
	if o == nil || openapi.IsNil(o.SharingKeyUl.Get()) {
		var ret int32
		return ret
	}
	return *o.SharingKeyUl.Get()
}

// GetSharingKeyUlOk returns a tuple with the SharingKeyUl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetSharingKeyUlOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.SharingKeyUl.Get(), o.SharingKeyUl.IsSet()
}

// HasSharingKeyUl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasSharingKeyUl() bool {
	if o != nil && o.SharingKeyUl.IsSet() {
		return true
	}

	return false
}

// SetSharingKeyUl gets a reference to the given openapi.NullableInt32 and assigns it to the SharingKeyUl field.
func (o *MediaComponentRm) SetSharingKeyUl(v int32) {
	o.SharingKeyUl.Set(&v)
}

// SetSharingKeyUlNil sets the value for SharingKeyUl to be an explicit nil
func (o *MediaComponentRm) SetSharingKeyUlNil() {
	o.SharingKeyUl.Set(nil)
}

// UnsetSharingKeyUl ensures that no value is present for SharingKeyUl, not even an explicit nil
func (o *MediaComponentRm) UnsetSharingKeyUl() {
	o.SharingKeyUl.Unset()
}

// GetTsnQos returns the TsnQos field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetTsnQos() TsnQosContainerRm {
	if o == nil || openapi.IsNil(o.TsnQos.Get()) {
		var ret TsnQosContainerRm
		return ret
	}
	return *o.TsnQos.Get()
}

// GetTsnQosOk returns a tuple with the TsnQos field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetTsnQosOk() (*TsnQosContainerRm, bool) {
	if o == nil {
		return nil, false
	}
	return o.TsnQos.Get(), o.TsnQos.IsSet()
}

// HasTsnQos returns a boolean if a field has been set.
func (o *MediaComponentRm) HasTsnQos() bool {
	if o != nil && o.TsnQos.IsSet() {
		return true
	}

	return false
}

// SetTsnQos gets a reference to the given NullableTsnQosContainerRm and assigns it to the TsnQos field.
func (o *MediaComponentRm) SetTsnQos(v TsnQosContainerRm) {
	o.TsnQos.Set(&v)
}

// SetTsnQosNil sets the value for TsnQos to be an explicit nil
func (o *MediaComponentRm) SetTsnQosNil() {
	o.TsnQos.Set(nil)
}

// UnsetTsnQos ensures that no value is present for TsnQos, not even an explicit nil
func (o *MediaComponentRm) UnsetTsnQos() {
	o.TsnQos.Unset()
}

// GetTscaiInputDl returns the TscaiInputDl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetTscaiInputDl() TscaiInputContainer {
	if o == nil || openapi.IsNil(o.TscaiInputDl.Get()) {
		var ret TscaiInputContainer
		return ret
	}
	return *o.TscaiInputDl.Get()
}

// GetTscaiInputDlOk returns a tuple with the TscaiInputDl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetTscaiInputDlOk() (*TscaiInputContainer, bool) {
	if o == nil {
		return nil, false
	}
	return o.TscaiInputDl.Get(), o.TscaiInputDl.IsSet()
}

// HasTscaiInputDl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasTscaiInputDl() bool {
	if o != nil && o.TscaiInputDl.IsSet() {
		return true
	}

	return false
}

// SetTscaiInputDl gets a reference to the given NullableTscaiInputContainer and assigns it to the TscaiInputDl field.
func (o *MediaComponentRm) SetTscaiInputDl(v TscaiInputContainer) {
	o.TscaiInputDl.Set(&v)
}

// SetTscaiInputDlNil sets the value for TscaiInputDl to be an explicit nil
func (o *MediaComponentRm) SetTscaiInputDlNil() {
	o.TscaiInputDl.Set(nil)
}

// UnsetTscaiInputDl ensures that no value is present for TscaiInputDl, not even an explicit nil
func (o *MediaComponentRm) UnsetTscaiInputDl() {
	o.TscaiInputDl.Unset()
}

// GetTscaiInputUl returns the TscaiInputUl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetTscaiInputUl() TscaiInputContainer {
	if o == nil || openapi.IsNil(o.TscaiInputUl.Get()) {
		var ret TscaiInputContainer
		return ret
	}
	return *o.TscaiInputUl.Get()
}

// GetTscaiInputUlOk returns a tuple with the TscaiInputUl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetTscaiInputUlOk() (*TscaiInputContainer, bool) {
	if o == nil {
		return nil, false
	}
	return o.TscaiInputUl.Get(), o.TscaiInputUl.IsSet()
}

// HasTscaiInputUl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasTscaiInputUl() bool {
	if o != nil && o.TscaiInputUl.IsSet() {
		return true
	}

	return false
}

// SetTscaiInputUl gets a reference to the given NullableTscaiInputContainer and assigns it to the TscaiInputUl field.
func (o *MediaComponentRm) SetTscaiInputUl(v TscaiInputContainer) {
	o.TscaiInputUl.Set(&v)
}

// SetTscaiInputUlNil sets the value for TscaiInputUl to be an explicit nil
func (o *MediaComponentRm) SetTscaiInputUlNil() {
	o.TscaiInputUl.Set(nil)
}

// UnsetTscaiInputUl ensures that no value is present for TscaiInputUl, not even an explicit nil
func (o *MediaComponentRm) UnsetTscaiInputUl() {
	o.TscaiInputUl.Unset()
}

// GetTscaiTimeDom returns the TscaiTimeDom field value if set, zero value otherwise.
func (o *MediaComponentRm) GetTscaiTimeDom() int32 {
	if o == nil || openapi.IsNil(o.TscaiTimeDom) {
		var ret int32
		return ret
	}
	return *o.TscaiTimeDom
}

// GetTscaiTimeDomOk returns a tuple with the TscaiTimeDom field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetTscaiTimeDomOk() (*int32, bool) {
	if o == nil || openapi.IsNil(o.TscaiTimeDom) {
		return nil, false
	}
	return o.TscaiTimeDom, true
}

// HasTscaiTimeDom returns a boolean if a field has been set.
func (o *MediaComponentRm) HasTscaiTimeDom() bool {
	if o != nil && !openapi.IsNil(o.TscaiTimeDom) {
		return true
	}

	return false
}

// SetTscaiTimeDom gets a reference to the given int32 and assigns it to the TscaiTimeDom field.
func (o *MediaComponentRm) SetTscaiTimeDom(v int32) {
	o.TscaiTimeDom = &v
}

// GetCapBatAdaptation returns the CapBatAdaptation field value if set, zero value otherwise.
func (o *MediaComponentRm) GetCapBatAdaptation() bool {
	if o == nil || openapi.IsNil(o.CapBatAdaptation) {
		var ret bool
		return ret
	}
	return *o.CapBatAdaptation
}

// GetCapBatAdaptationOk returns a tuple with the CapBatAdaptation field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetCapBatAdaptationOk() (*bool, bool) {
	if o == nil || openapi.IsNil(o.CapBatAdaptation) {
		return nil, false
	}
	return o.CapBatAdaptation, true
}

// HasCapBatAdaptation returns a boolean if a field has been set.
func (o *MediaComponentRm) HasCapBatAdaptation() bool {
	if o != nil && !openapi.IsNil(o.CapBatAdaptation) {
		return true
	}

	return false
}

// SetCapBatAdaptation gets a reference to the given bool and assigns it to the CapBatAdaptation field.
func (o *MediaComponentRm) SetCapBatAdaptation(v bool) {
	o.CapBatAdaptation = &v
}

// GetRTLatencyInd returns the RTLatencyInd field value if set, zero value otherwise.
func (o *MediaComponentRm) GetRTLatencyInd() bool {
	if o == nil || openapi.IsNil(o.RTLatencyInd) {
		var ret bool
		return ret
	}
	return *o.RTLatencyInd
}

// GetRTLatencyIndOk returns a tuple with the RTLatencyInd field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetRTLatencyIndOk() (*bool, bool) {
	if o == nil || openapi.IsNil(o.RTLatencyInd) {
		return nil, false
	}
	return o.RTLatencyInd, true
}

// HasRTLatencyInd returns a boolean if a field has been set.
func (o *MediaComponentRm) HasRTLatencyInd() bool {
	if o != nil && !openapi.IsNil(o.RTLatencyInd) {
		return true
	}

	return false
}

// SetRTLatencyInd gets a reference to the given bool and assigns it to the RTLatencyInd field.
func (o *MediaComponentRm) SetRTLatencyInd(v bool) {
	o.RTLatencyInd = &v
}

// GetPduSetQosDl returns the PduSetQosDl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetPduSetQosDl() PduSetQosParaRm {
	if o == nil || openapi.IsNil(o.PduSetQosDl.Get()) {
		var ret PduSetQosParaRm
		return ret
	}
	return *o.PduSetQosDl.Get()
}

// GetPduSetQosDlOk returns a tuple with the PduSetQosDl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetPduSetQosDlOk() (*PduSetQosParaRm, bool) {
	if o == nil {
		return nil, false
	}
	return o.PduSetQosDl.Get(), o.PduSetQosDl.IsSet()
}

// HasPduSetQosDl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasPduSetQosDl() bool {
	if o != nil && o.PduSetQosDl.IsSet() {
		return true
	}

	return false
}

// SetPduSetQosDl gets a reference to the given NullablePduSetQosParaRm and assigns it to the PduSetQosDl field.
func (o *MediaComponentRm) SetPduSetQosDl(v PduSetQosParaRm) {
	o.PduSetQosDl.Set(&v)
}

// SetPduSetQosDlNil sets the value for PduSetQosDl to be an explicit nil
func (o *MediaComponentRm) SetPduSetQosDlNil() {
	o.PduSetQosDl.Set(nil)
}

// UnsetPduSetQosDl ensures that no value is present for PduSetQosDl, not even an explicit nil
func (o *MediaComponentRm) UnsetPduSetQosDl() {
	o.PduSetQosDl.Unset()
}

// GetPduSetQosUl returns the PduSetQosUl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *MediaComponentRm) GetPduSetQosUl() PduSetQosParaRm {
	if o == nil || openapi.IsNil(o.PduSetQosUl.Get()) {
		var ret PduSetQosParaRm
		return ret
	}
	return *o.PduSetQosUl.Get()
}

// GetPduSetQosUlOk returns a tuple with the PduSetQosUl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *MediaComponentRm) GetPduSetQosUlOk() (*PduSetQosParaRm, bool) {
	if o == nil {
		return nil, false
	}
	return o.PduSetQosUl.Get(), o.PduSetQosUl.IsSet()
}

// HasPduSetQosUl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasPduSetQosUl() bool {
	if o != nil && o.PduSetQosUl.IsSet() {
		return true
	}

	return false
}

// SetPduSetQosUl gets a reference to the given NullablePduSetQosParaRm and assigns it to the PduSetQosUl field.
func (o *MediaComponentRm) SetPduSetQosUl(v PduSetQosParaRm) {
	o.PduSetQosUl.Set(&v)
}

// SetPduSetQosUlNil sets the value for PduSetQosUl to be an explicit nil
func (o *MediaComponentRm) SetPduSetQosUlNil() {
	o.PduSetQosUl.Set(nil)
}

// UnsetPduSetQosUl ensures that no value is present for PduSetQosUl, not even an explicit nil
func (o *MediaComponentRm) UnsetPduSetQosUl() {
	o.PduSetQosUl.Unset()
}

// GetProtoDescDl returns the ProtoDescDl field value if set, zero value otherwise.
func (o *MediaComponentRm) GetProtoDescDl() ProtocolDescription {
	if o == nil || openapi.IsNil(o.ProtoDescDl) {
		var ret ProtocolDescription
		return ret
	}
	return *o.ProtoDescDl
}

// GetProtoDescDlOk returns a tuple with the ProtoDescDl field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetProtoDescDlOk() (*ProtocolDescription, bool) {
	if o == nil || openapi.IsNil(o.ProtoDescDl) {
		return nil, false
	}
	return o.ProtoDescDl, true
}

// HasProtoDescDl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasProtoDescDl() bool {
	if o != nil && !openapi.IsNil(o.ProtoDescDl) {
		return true
	}

	return false
}

// SetProtoDescDl gets a reference to the given ProtocolDescription and assigns it to the ProtoDescDl field.
func (o *MediaComponentRm) SetProtoDescDl(v ProtocolDescription) {
	o.ProtoDescDl = &v
}

// GetProtoDescUl returns the ProtoDescUl field value if set, zero value otherwise.
func (o *MediaComponentRm) GetProtoDescUl() ProtocolDescription {
	if o == nil || openapi.IsNil(o.ProtoDescUl) {
		var ret ProtocolDescription
		return ret
	}
	return *o.ProtoDescUl
}

// GetProtoDescUlOk returns a tuple with the ProtoDescUl field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetProtoDescUlOk() (*ProtocolDescription, bool) {
	if o == nil || openapi.IsNil(o.ProtoDescUl) {
		return nil, false
	}
	return o.ProtoDescUl, true
}

// HasProtoDescUl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasProtoDescUl() bool {
	if o != nil && !openapi.IsNil(o.ProtoDescUl) {
		return true
	}

	return false
}

// SetProtoDescUl gets a reference to the given ProtocolDescription and assigns it to the ProtoDescUl field.
func (o *MediaComponentRm) SetProtoDescUl(v ProtocolDescription) {
	o.ProtoDescUl = &v
}

// GetPeriodUl returns the PeriodUl field value if set, zero value otherwise.
func (o *MediaComponentRm) GetPeriodUl() int32 {
	if o == nil || openapi.IsNil(o.PeriodUl) {
		var ret int32
		return ret
	}
	return *o.PeriodUl
}

// GetPeriodUlOk returns a tuple with the PeriodUl field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetPeriodUlOk() (*int32, bool) {
	if o == nil || openapi.IsNil(o.PeriodUl) {
		return nil, false
	}
	return o.PeriodUl, true
}

// HasPeriodUl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasPeriodUl() bool {
	if o != nil && !openapi.IsNil(o.PeriodUl) {
		return true
	}

	return false
}

// SetPeriodUl gets a reference to the given int32 and assigns it to the PeriodUl field.
func (o *MediaComponentRm) SetPeriodUl(v int32) {
	o.PeriodUl = &v
}

// GetPeriodDl returns the PeriodDl field value if set, zero value otherwise.
func (o *MediaComponentRm) GetPeriodDl() int32 {
	if o == nil || openapi.IsNil(o.PeriodDl) {
		var ret int32
		return ret
	}
	return *o.PeriodDl
}

// GetPeriodDlOk returns a tuple with the PeriodDl field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetPeriodDlOk() (*int32, bool) {
	if o == nil || openapi.IsNil(o.PeriodDl) {
		return nil, false
	}
	return o.PeriodDl, true
}

// HasPeriodDl returns a boolean if a field has been set.
func (o *MediaComponentRm) HasPeriodDl() bool {
	if o != nil && !openapi.IsNil(o.PeriodDl) {
		return true
	}

	return false
}

// SetPeriodDl gets a reference to the given int32 and assigns it to the PeriodDl field.
func (o *MediaComponentRm) SetPeriodDl(v int32) {
	o.PeriodDl = &v
}

// GetL4sInd returns the L4sInd field value if set, zero value otherwise.
func (o *MediaComponentRm) GetL4sInd() UplinkDownlinkSupport {
	if o == nil || openapi.IsNil(o.L4sInd) {
		var ret UplinkDownlinkSupport
		return ret
	}
	return *o.L4sInd
}

// GetL4sIndOk returns a tuple with the L4sInd field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaComponentRm) GetL4sIndOk() (*UplinkDownlinkSupport, bool) {
	if o == nil || openapi.IsNil(o.L4sInd) {
		return nil, false
	}
	return o.L4sInd, true
}

// HasL4sInd returns a boolean if a field has been set.
func (o *MediaComponentRm) HasL4sInd() bool {
	if o != nil && !openapi.IsNil(o.L4sInd) {
		return true
	}

	return false
}

// SetL4sInd gets a reference to the given UplinkDownlinkSupport and assigns it to the L4sInd field.
func (o *MediaComponentRm) SetL4sInd(v UplinkDownlinkSupport) {
	o.L4sInd = &v
}

func (o MediaComponentRm) ToMap() (map[string]any, error) {
	toSerialize := map[string]any{}
	if !openapi.IsNil(o.AfAppId) {
		toSerialize["afAppId"] = o.AfAppId
	}
	if o.AfRoutReq.IsSet() {
		toSerialize["afRoutReq"] = o.AfRoutReq.Get()
	}
	if o.AfSfcReq.IsSet() {
		toSerialize["afSfcReq"] = o.AfSfcReq.Get()
	}
	if o.QosReference.IsSet() {
		toSerialize["qosReference"] = o.QosReference.Get()
	}
	if o.AltSerReqs != nil {
		toSerialize["altSerReqs"] = o.AltSerReqs
	}
	if o.AltSerReqsData != nil {
		toSerialize["altSerReqsData"] = o.AltSerReqsData
	}
	if !openapi.IsNil(o.DisUeNotif) {
		toSerialize["disUeNotif"] = o.DisUeNotif
	}
	if !openapi.IsNil(o.ContVer) {
		toSerialize["contVer"] = o.ContVer
	}
	if !openapi.IsNil(o.Codecs) {
		toSerialize["codecs"] = o.Codecs
	}
	if o.DesMaxLatency.IsSet() {
		toSerialize["desMaxLatency"] = o.DesMaxLatency.Get()
	}
	if o.DesMaxLoss.IsSet() {
		toSerialize["desMaxLoss"] = o.DesMaxLoss.Get()
	}
	if o.FlusId.IsSet() {
		toSerialize["flusId"] = o.FlusId.Get()
	}
	if !openapi.IsNil(o.FStatus) {
		toSerialize["fStatus"] = o.FStatus
	}
	if o.MarBwDl.IsSet() {
		toSerialize["marBwDl"] = o.MarBwDl.Get()
	}
	if o.MarBwUl.IsSet() {
		toSerialize["marBwUl"] = o.MarBwUl.Get()
	}
	if o.MaxPacketLossRateDl.IsSet() {
		toSerialize["maxPacketLossRateDl"] = o.MaxPacketLossRateDl.Get()
	}
	if o.MaxPacketLossRateUl.IsSet() {
		toSerialize["maxPacketLossRateUl"] = o.MaxPacketLossRateUl.Get()
	}
	if o.MaxSuppBwDl.IsSet() {
		toSerialize["maxSuppBwDl"] = o.MaxSuppBwDl.Get()
	}
	if o.MaxSuppBwUl.IsSet() {
		toSerialize["maxSuppBwUl"] = o.MaxSuppBwUl.Get()
	}
	toSerialize["medCompN"] = o.MedCompN
	if !openapi.IsNil(o.MedSubComps) {
		toSerialize["medSubComps"] = o.MedSubComps
	}
	if !openapi.IsNil(o.MedType) {
		toSerialize["medType"] = o.MedType
	}
	if o.MinDesBwDl.IsSet() {
		toSerialize["minDesBwDl"] = o.MinDesBwDl.Get()
	}
	if o.MinDesBwUl.IsSet() {
		toSerialize["minDesBwUl"] = o.MinDesBwUl.Get()
	}
	if o.MirBwDl.IsSet() {
		toSerialize["mirBwDl"] = o.MirBwDl.Get()
	}
	if o.MirBwUl.IsSet() {
		toSerialize["mirBwUl"] = o.MirBwUl.Get()
	}
	if !openapi.IsNil(o.PreemptCap) {
		toSerialize["preemptCap"] = o.PreemptCap
	}
	if !openapi.IsNil(o.PreemptVuln) {
		toSerialize["preemptVuln"] = o.PreemptVuln
	}
	if !openapi.IsNil(o.PrioSharingInd) {
		toSerialize["prioSharingInd"] = o.PrioSharingInd
	}
	if !openapi.IsNil(o.ResPrio) {
		toSerialize["resPrio"] = o.ResPrio
	}
	if o.RrBw.IsSet() {
		toSerialize["rrBw"] = o.RrBw.Get()
	}
	if o.RsBw.IsSet() {
		toSerialize["rsBw"] = o.RsBw.Get()
	}
	if o.SharingKeyDl.IsSet() {
		toSerialize["sharingKeyDl"] = o.SharingKeyDl.Get()
	}
	if o.SharingKeyUl.IsSet() {
		toSerialize["sharingKeyUl"] = o.SharingKeyUl.Get()
	}
	if o.TsnQos.IsSet() {
		toSerialize["tsnQos"] = o.TsnQos.Get()
	}
	if o.TscaiInputDl.IsSet() {
		toSerialize["tscaiInputDl"] = o.TscaiInputDl.Get()
	}
	if o.TscaiInputUl.IsSet() {
		toSerialize["tscaiInputUl"] = o.TscaiInputUl.Get()
	}
	if !openapi.IsNil(o.TscaiTimeDom) {
		toSerialize["tscaiTimeDom"] = o.TscaiTimeDom
	}
	if !openapi.IsNil(o.CapBatAdaptation) {
		toSerialize["capBatAdaptation"] = o.CapBatAdaptation
	}
	if !openapi.IsNil(o.RTLatencyInd) {
		toSerialize["rTLatencyInd"] = o.RTLatencyInd
	}
	if o.PduSetQosDl.IsSet() {
		toSerialize["pduSetQosDl"] = o.PduSetQosDl.Get()
	}
	if o.PduSetQosUl.IsSet() {
		toSerialize["pduSetQosUl"] = o.PduSetQosUl.Get()
	}
	if !openapi.IsNil(o.ProtoDescDl) {
		toSerialize["protoDescDl"] = o.ProtoDescDl
	}
	if !openapi.IsNil(o.ProtoDescUl) {
		toSerialize["protoDescUl"] = o.ProtoDescUl
	}
	if !openapi.IsNil(o.PeriodUl) {
		toSerialize["periodUl"] = o.PeriodUl
	}
	if !openapi.IsNil(o.PeriodDl) {
		toSerialize["periodDl"] = o.PeriodDl
	}
	if !openapi.IsNil(o.L4sInd) {
		toSerialize["l4sInd"] = o.L4sInd
	}
	return toSerialize, nil
}

type NullableMediaComponentRm struct {
	value *MediaComponentRm
	isSet bool
}

func (v NullableMediaComponentRm) Get() *MediaComponentRm {
	return v.value
}

func (v *NullableMediaComponentRm) Set(val *MediaComponentRm) {
	v.value = val
	v.isSet = true
}

func (v NullableMediaComponentRm) IsSet() bool {
	return v.isSet
}

func (v *NullableMediaComponentRm) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableMediaComponentRm(val *MediaComponentRm) *NullableMediaComponentRm {
	return &NullableMediaComponentRm{value: val, isSet: true}
}

func (v NullableMediaComponentRm) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableMediaComponentRm) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
