// Copyright (C) 2026 Intel Corporation
// SPDX-License-Identifier: Apache-2.0

/*
Namf_Communication

AMF Communication Service.
© 2024, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).
All rights reserved.

API version: 1.3.0-alpha.6

Authors: Aether SD-Core team
Contact: dev@lists.aetherproject.org

Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.
*/

package models

import (
	"encoding/json"

	"github.com/omec-project/openapi/v2"
)

// checks if the SmfChangeInfo type satisfies the MappedNullable interface at compile time
var _ openapi.MappedNullable = &SmfChangeInfo{}

// SmfChangeInfo SMF change information for PDU session(s)
type SmfChangeInfo struct {
	PduSessionIdList []int32             `json:"pduSessionIdList" yaml:"pduSessionIdList"`
	SmfChangeInd     SmfChangeIndication `json:"smfChangeInd" yaml:"smfChangeInd"`
}

// NewSmfChangeInfo instantiates a new SmfChangeInfo object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSmfChangeInfo(pduSessionIdList []int32, smfChangeInd SmfChangeIndication) *SmfChangeInfo {
	this := SmfChangeInfo{}
	this.PduSessionIdList = pduSessionIdList
	this.SmfChangeInd = smfChangeInd
	return &this
}

// NewSmfChangeInfoWithDefaults instantiates a new SmfChangeInfo object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSmfChangeInfoWithDefaults() *SmfChangeInfo {
	this := SmfChangeInfo{}
	return &this
}

// GetPduSessionIdList returns the PduSessionIdList field value
func (o *SmfChangeInfo) GetPduSessionIdList() []int32 {
	if o == nil {
		var ret []int32
		return ret
	}

	return o.PduSessionIdList
}

// GetPduSessionIdListOk returns a tuple with the PduSessionIdList field value
// and a boolean to check if the value has been set.
func (o *SmfChangeInfo) GetPduSessionIdListOk() ([]int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.PduSessionIdList, true
}

// SetPduSessionIdList sets field value
func (o *SmfChangeInfo) SetPduSessionIdList(v []int32) {
	o.PduSessionIdList = v
}

// GetSmfChangeInd returns the SmfChangeInd field value
func (o *SmfChangeInfo) GetSmfChangeInd() SmfChangeIndication {
	if o == nil {
		var ret SmfChangeIndication
		return ret
	}

	return o.SmfChangeInd
}

// GetSmfChangeIndOk returns a tuple with the SmfChangeInd field value
// and a boolean to check if the value has been set.
func (o *SmfChangeInfo) GetSmfChangeIndOk() (*SmfChangeIndication, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SmfChangeInd, true
}

// SetSmfChangeInd sets field value
func (o *SmfChangeInfo) SetSmfChangeInd(v SmfChangeIndication) {
	o.SmfChangeInd = v
}

func (o SmfChangeInfo) ToMap() (map[string]any, error) {
	toSerialize := map[string]any{}
	toSerialize["pduSessionIdList"] = o.PduSessionIdList
	toSerialize["smfChangeInd"] = o.SmfChangeInd
	return toSerialize, nil
}

type NullableSmfChangeInfo struct {
	value *SmfChangeInfo
	isSet bool
}

func (v NullableSmfChangeInfo) Get() *SmfChangeInfo {
	return v.value
}

func (v *NullableSmfChangeInfo) Set(val *SmfChangeInfo) {
	v.value = val
	v.isSet = true
}

func (v NullableSmfChangeInfo) IsSet() bool {
	return v.isSet
}

func (v *NullableSmfChangeInfo) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSmfChangeInfo(val *SmfChangeInfo) *NullableSmfChangeInfo {
	return &NullableSmfChangeInfo{value: val, isSet: true}
}

func (v NullableSmfChangeInfo) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSmfChangeInfo) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
